...
A Plan Bee authored via `/quo-plan-from-specs --feature "<title>"` contains
a single line of the literal form:

    Scoped to `### Feature: <title>` from <prd-path> and <sdd-path>.

When present, downstream skills (quo-breakdown-epic, quo-execute,
quo-fix-issue) must scope each named doc's content to the matching
