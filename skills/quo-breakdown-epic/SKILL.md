...
The `-research-` infix marks these dispatches as breakdown-time research. `/quo-execute` records its implementation-time dispatches in its run-state manifest's `## Lanes`, not in the TaskList, so a reader scanning the TaskList sees only breakdown-time entries, and the infix keeps them distinguishable from `gate-*` and `defer-*` tasks at a glance.
