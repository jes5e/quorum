...
in-flight emission contract. Every finding this sweep emits is a gate
candidate regardless of depth; whether its fix is then read by a reviewer
lane depends on what the fix changes (code, tests, or text), not on this
tag. Preserve the `file:line` +
