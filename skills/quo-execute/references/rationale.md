...
- The one pre-dispatch classification `/quo-fix-issue` makes, the `text` class, keys on what the change can touch — code behavior or a test outcome — not on the body's quality: a change that cannot break behavior gives a design read, a test pass, a PM pass, and a sweep nothing to catch, and one writer plus one reviewer is the hand baseline such an Issue is measured against. A writer whose `Kinds changed` names `code` or `tests` re-escalates the class, so a wrong judgment costs one dispatch, not coverage.
...
- A Revise re-dispatch hands the revising Analyst the prior proposal in full — in `/quo-fix-issue` by the path of the proposal file, the same bytes at a fraction of the prompt — including its `### Deferred refinements`, so it carries each deferral forward or drops it explicitly.
...
- In `/quo-fix-issue` the Design Proposal's durable carrier is the file the orchestrator writes when the Analyst's return reaches the gate, recorded on the manifest's `**Proposal:**` line: its random suffix keeps a stale file from another run from being read as this one's, and the open gate tells whether what it holds is approved. An execute revision has no carrier until its Approve appends it to its scope's ticket. Re-deriving through the Analyst is the recovery only when the carrier is missing, because a proposal reconstructed from a summary is a guess.
