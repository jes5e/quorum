...
- **Why two consecutive earned rounds escalate.** Each earned round means the previous fix left an outstanding code or test finding against it; two in a row means the implementer is patching without the rule that would make the fix right. Where a design authority exists it supplies that rule; where none does, the operator decides at the escalation gate, not another implementer.
