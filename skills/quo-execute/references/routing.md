...
- A `blocker` that cannot be made inapplicable to what ships without leaving the stated defect partly unfixed is in scope; it takes `Fix properly now` or `Re-dispatch the Analyst with this finding`, never `Defer`.
...
- The gate fires for a `blocker` like any other finding; `Fix properly now` against `Re-dispatch the Analyst with this finding` is a real decision, three choices when the Defer-with-narrowing branch is open.
...
- **Why gate (c) offers no `Cancel`.** Scope-bounding is a per-finding decision and a `Cancel` there would be ambiguous; the user retains `Ctrl-C` for run-level abort.
...
- **Why a contradicting finding goes to the Analyst first.** A reviewer finding that contradicts the approved design or a prior clean pass is either a real gap in the design or a false premise; dispatching an implementer settles neither, and on a false premise it starts a cascade of fixes to a bug that does not exist. One design-side read answers it. Both skills consult the Analyst — `/quo-fix-issue` from the proposal it approved, `/quo-execute` from the tickets and spec the plan left, a design done before any code landed and the one a mid-execution question needs re-read; asking the operator instead put the question to a human who had not read the code.
