...
**Killed lane.** An Agent killed or stopped before it returned — by the operator, the orchestrator, or the harness; a notification with no return reads as one — closes its lane and writes a `killed` ledger row (Section 2). Nothing it did is reported. Before the next dispatch, verify that every source path is as the last completed implementer pass left it. The perturbation protocol copies a file under `<tempdir>/.quorum/` before mutating it, so a copy written during the killed lane's run whose hash differs from the working-tree file's is a live perturbation; restore the file from the copy. A file with no copy is taken as untouched. Then re-dispatch the lane at the same round.
