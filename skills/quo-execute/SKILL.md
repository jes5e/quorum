...
- `## Rounds` holds one row per review lane, keyed by the scope and the **Reviews**-line slot (`review`) the row belongs to, not the implementer that applied its nits. `classification` lists one value per round after the first, in round order — `earned`, `late`, `variance`, `clean`, or `unavailable post-compaction` — and is empty until a second round is dispositioned. `decision` is the lane's latest exit decision: `another round`, or, when the lane closes, `close — clean` (nothing applied in a final pass), `close — nits only` (only `trivial-tweak` nits applied), or `close — residual` (at least one residual applied; it wins over `close — nits only` when both apply). The two counts are the `trivial-tweak` nits and the residual `suggestion`s applied in the lane's final implementer pass without a further reviewer round (Section 7). Rewrite the row when a round's findings are routed, before the action the decision names. `round` in `## Lanes` counts dispatches; `## Rounds` records decisions and counts.
...
- **Bee-level reviews.** Entered when every Epic is `done` (from run start or from the Epic boundary), or on a Mode 1 **Stop here**: dispatch up to three concurrent reviewer Agents at Bee scope: `Agent(subagent_type="code-reviewer", run_in_background=true)` if any Engineer ran, `Agent(subagent_type="test-reviewer", run_in_background=true)` if any Test Writer ran, `Agent(subagent_type="doc-reviewer", run_in_background=true)` if any Doc Writer ran. Follow each review skill's routing trailer literally, but dispatch no implementer for a Bee-level finding until every Bee-scoped reviewer and PM lane has returned, so no lane reads a moving tree. Re-dispatch implementers and, when needed, a PM at Bee scope; you may choose NOT to spawn the PM on a minor iteration. Findings routed from this site take Clause 2 of part (g). The loop has not closed while any Bee-scoped `aborted-*` obligation is `open`. When the loop closes and every Bee-scoped lane is `closed`, proceed to Section 11.
...
| `## Engineer's completeness evidence` | every Engineer completeness list for the scope, verbatim and inline, never as a file path, each attributed to its Subtask | PM (per-Task and Bee-scoped), Code Reviewer | omit the heading when no list arrived; always state separately that the assignment was sweep-shaped — a directive or ticket body that directed a change at every site where some property holds — when it was; at Bee scope name every sweep-shaped Subtask assignment from the Epic loop, and name the Subtask whose list `existed but is unavailable post-compaction` when that is so |
...
**Severity bounds the loop.** A review lane's loop — the Engineer → code-review loop at any review site, and each writer → reviewer pair — is held open by `blocker` and `suggestion` findings and by any `nit` whose chosen path is deeper than `trivial-tweak`. It closes when no such finding remains outstanding. Those are the lane-holding findings: the tag fixes the set, and the residual clause below is its one override. Severity is consequence, orthogonal to depth; the levels and their one-line tests live in the routing reference (Section 3) and in the review skills.

- A lane-holding finding stays outstanding until a later cold pass no longer raises it, or it reaches a disposition under which **no fix lands against it**. Those dispositions are: accepted into the compromise tracker; recorded as a `defer-*` obligation; closed by `Accept the limitation`, `Cancel`, or a `Defer to follow-up Issue` that ships nothing this round. A dispatch settles nothing — ungated at row 6, chosen at a gate, or the soft fix or narrowing a deferral ships alongside itself — until a later cold pass has read its result.
- **A `nit` whose chosen path is a `trivial-tweak` never reopens a lane on its own.** When a lane-holding finding forces another implementer round, such nits ride along and the next cold pass reads them. When only such nits remain, apply them in **one** final implementer pass for that lane and dispatch **no** further reviewer round. At a site where the review runs inside the PM's in-flight passes, that means do not re-dispatch the PM for that lane.
- **Residual clause.** A `suggestion` closes with the lane, applied in that final pass, only when both hold. First, its chosen fix path changes neither what the text asserts nor what the code does — the `nit` test, judged over the reviewer's own path description. Second, a prior cold pass at this lane read the text it targets and did not flag it, and no implementer pass at this lane since was dispatched to change it. Judge the second condition over your own dispatch prompts and the prior returns, never over file contents; when either is unreadable the clause does not fire. It cannot fire at a lane's first round, and never applies to a `blocker` or a depth-held `nit`.
- **Exit decision per round.** When a round's findings are routed, rewrite the lane's `## Rounds` row (Section 2) — classification, decision, counts — before the action the decision names: the re-dispatch, the final implementer pass, or marking the lane `closed`. Classify a round after the first by the first match in this list. A return that is itself unreadable is re-dispatched at the same round, never decided from a summary.
  - `clean` — the round raised no lane-holding finding, released or not.
  - `variance` — every lane-holding finding it raised was a `suggestion` the residual clause released.
  - `unavailable post-compaction` — the prior returns or dispatch prompts are gone.
  - `earned` — a lane-holding finding the clause did not release targets the previous round's fix.
  - `late` — otherwise: such a finding sits on older text no prior pass flagged.
- Render per lane, in its slot on the **Reviews** line of the summary for the scope that dispatched the review. Render `N rounds (earned/late/variance/unavailable = a/b/c/d)` when the lane ran more than once, then `N nits applied without re-review`, then `N residuals applied without re-review`. `N` is the lane's round count and `d` counts rounds classified `unavailable post-compaction`; the remainder is the first round plus the `clean` rounds, and an unavailable round is never rendered as `clean`. Follow the residual count with each residual's description on one line, or with `descriptions unavailable post-compaction` when the return that raised it is gone. When a lane that ran carries no row in the manifest render `count unavailable post-compaction` rather than guessing. Applied nits and residuals are finished work: never ignored feedback, never `defer-*` obligations.
- A `nit` whose chosen fix path would fire a gate under part (a) still routes through the table like any other finding. **The bound keys on what ships**: when the gate's answer ships anything other than the enumerated `trivial-tweak` path (`Fix properly now` on a row-2, -3, or -4 fire, or the soft fix or narrowing a `Defer` ships alongside itself), the finding is lane-holding again. A cold pass must read that fix, and part (g)'s elision does not apply to it. When the final pass ships only such `trivial-tweak` paths and residuals and changes source, part (g)'s ordering applies with its code-review rung elided — that rung is the one round this rule suppresses.
...
**(g) Re-dispatch ordering when a fix path changes source.** Dispatch the Engineer first, then the code review for that site against the resulting diff. Only once that review has closed, dispatch the Test Writer and/or Doc Writer the change invalidated. The rounds are ordered, not concurrent, and gated by the Engineer-dispatch precondition (Section 6). One elision: on the final pass that ships only `trivial-tweak` nit paths and residuals, skip the code-review rung and dispatch the affected writer once the Engineer returns. A path that changes no source file re-dispatches that single writer lane alone.
...
**Reviews**: [Code review: X issues found/None needed | Docs review: Y issues found/None needed — each slot carrying its own `N rounds (earned/late/variance/unavailable = a/b/c/d)` when the lane ran more than once, and its `N nits applied without re-review` and `N residuals applied without re-review — <one line per residual>` when any (or "count unavailable post-compaction" / "descriptions unavailable post-compaction"), attributed by review site where a slot has more than one]
...
6. Dispose per the answer:
   - **Fix in this session**: per-unit rules apply with `postcomp-<n>` as the unit, `<n>` the finding's 1-based index: lanes at scope `postcomp-<n>`, the movement rung, and part (g) ordering within a finding. Findings whose fixes touch provably disjoint files run concurrently; all others run one at a time, in index order, each at its own `postcomp-<n>` scope. The Engineer-dispatch precondition holds across every `postcomp-*` scope: a writer, reviewer, or PM lane waits for every `open` Engineer lane regardless of file overlap. An abort of a post-completion lane closes its rows here and continues this section; it never enters Section 12. A `blocker` or `suggestion` finding's fix — and a `nit`'s whose chosen path is deeper than `trivial-tweak` — is read by the reviewer lane its implementer maps to (Engineer → Code Reviewer, Test Writer → Test Reviewer, Doc Writer → Doc Reviewer) at scope `postcomp-<n>`, under Section 7's bound and with its own `## Rounds` row; a `trivial-tweak` `nit` finding gets its implementer pass and no reviewer.
...
**Bee Status**: Finished
**Reviews**: [Bee-level code review: X issues found/None needed | Test review: Y issues found/None needed | Docs review: Z issues found/None needed — each slot carrying its own `N rounds (earned/late/variance/unavailable = a/b/c/d)` when the lane ran more than once, and its `N nits applied without re-review` and `N residuals applied without re-review — <one line per residual>` when any (or "count unavailable post-compaction" / "descriptions unavailable post-compaction"), attributed by review site where a slot has more than one]
