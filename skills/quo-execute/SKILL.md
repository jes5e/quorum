...
1. Mark `closed` every lane and `aborted-*` obligation at the aborted scope — the Task and its Subtask scopes on a mid-Epic abort, every Bee scope on a Bee-level abort — recording the abort reason in each `aborted-*` obligation's detail. An in-flight sibling lane cannot be terminated; mark it `closed` anyway.
