...
| `## Engineer's diff (path)` | the path of a file written just before the dispatch with `git diff <pre-bee-sha> -- <the scope's fingerprint-set paths> --output=<tempdir>/.quorum/diff-<scope-id>-<short-suffix>.md` — one literal command, identical on POSIX and PowerShell once the `.quorum` directory exists per Section 2 | Doc Writer | omit when no Engineer ran at the scope |
...
| `## Site enumeration requested` | the fixed line `List every site this unit's change introduced and whether the artifact under review covers it; return the gaps as one finding.` | the reviewer of a lane whose `## Rounds` row shows two consecutive `late` classifications (Section 7) | omit otherwise |
...
- **Late-chain enumeration.** When the lane's `## Rounds` row shows two consecutive `late` classifications, its next review is the confirming pass plus `## Site enumeration requested` (Section 5): the reviewer lists every site the unit's change introduced and whether the artifact under review covers each, and returns the gaps as one finding. One implementer pass closes every listed gap; the lane then closes with decision `close — enumerated` and the post-completion review reads it — unless that pass changed `code`, which owes its confirming pass under the hold set. An empty gap list closes the lane `close — clean`; an unreadable enumeration return is a full pass.
...
- Render per lane, in its slot on the **Reviews** line of the summary for the scope that dispatched the review. Render `N rounds (earned/late/text/unavailable = a/b/c/d)` when the lane ran more than once, then `N text fixes applied without re-review`. `N` is the lane's round count and `d` counts rounds classified `unavailable post-compaction`; the remainder is the first round plus the `clean` rounds, and an unavailable round is never rendered as `clean`. When a lane that ran carries no row in the manifest render `count unavailable post-compaction` rather than guessing.
