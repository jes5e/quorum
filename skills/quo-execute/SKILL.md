...
## Rounds
| scope | review | nits applied without re-review |
|---|---|---|
...
- `## Rounds` holds one row per review lane, recording the scope and the **Reviews**-line slot the nits belong to, not the implementer that applied them. The row carries the count of `trivial-tweak` nits applied in that lane's final implementer pass without a further reviewer round (Section 7). `round` in `## Lanes` counts dispatches; `## Rounds` counts nits.
...
**Reviews**: [Code review: X issues found/None needed | Docs review: Y issues found/None needed — each slot carrying its own `N nits applied without re-review` (or "count unavailable post-compaction") when any, attributed by review site where a slot has more than one]
...
**Bee Status**: Finished
**Reviews**: [Bee-level code review: X issues found/None needed | Test review: Y issues found/None needed | Docs review: Z issues found/None needed — each slot carrying its own `N nits applied without re-review` (or "count unavailable post-compaction") when any, attributed by review site where a slot has more than one]
