...
- Roles: the per-Subtask implementers `agents/engineer.md`, `agents/test-writer.md`, `agents/doc-writer.md`; the reviewer `agents/pm.md`; the review-site reviewers `agents/code-reviewer.md`, `agents/test-reviewer.md`, `agents/doc-reviewer.md`. Role contracts live there; dispatch roles, do not carry their prose.
...
- **Escalation.** Fires from Section 7's earned-chain escalation, because this skill dispatches no Analyst and the operator is its design authority. The question carries the chain verbatim: `Lane <slot> at <scope> has had two consecutive rounds that each raised an outstanding code or test finding against the previous round's fix. How should it proceed?` Options: **Continue** — resume the lane's implementer (Section 5) for the chain, carrying any direction you give in prose; **Stop the unit** — route through Section 12 with the chain reported; at a `postcomp-<n>` scope, close that lane's rows and continue Section 11 instead, since a post-completion lane never enters Section 12.
...
**Killed lane.** An Agent killed or stopped before it returned — by the operator, the orchestrator, or the harness; a notification with no return reads as one — closes its lane and writes a `killed` ledger row (Section 2) at the moment the orchestrator learns of the kill. Nothing it did is reported. Before the next dispatch, verify that the production region is as the last completed implementer pass left it. The records are the copies the perturbation protocol writes under `<tempdir>/.quorum/` before mutating a file, named for the file's repository-relative path (copies older than `**Run started (UTC):**` belong to earlier runs), and the latest `## Engineer's diff (path)` file. Restore a perturbed file from its copy; when a difference could be landed work rather than a perturbation, stop and tell the operator, because a restore from a stale copy destroys an implementer's edit. A file with no copy is taken as untouched. Then re-dispatch the lane; the killed dispatch is a `## Lanes` row and decides nothing in `## Rounds`.

A killed implementer's own edits have no copy. Re-dispatch it against the current tree — a resume, or fresh when the send fails — with the routed findings again and a statement that its previous pass was cut off with no `## Files changed`, so the diff may hold a partial edit to finish or undo first. The compile rung and the confirming pass a `code` pass owes are the backstops; the run that motivated this rule had two lanes read a perturbed tree before anyone checked.
...
**Cost**: [N dispatches, T subagent tokens — the ledger rows at this Task's scopes, `killed` rows excluded from N]
...
Entered from **Abort this unit**, from `Cancel` at either routing gate, or from **Stop the unit** at the escalation gate. A definition, not a step; run it only when a branch names it. The run stops without the unit advancing.
...
**Cost**: [N dispatches, T subagent tokens, wall clock — every ledger row, `killed` rows excluded from N; the wall clock runs from **Run started (UTC):** to the current UTC time]
