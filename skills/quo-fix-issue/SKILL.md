...
- `**Context guard:**` starts at `not run` and is rewritten only by the guard (Section 8).
...
- Validate the manifest before trusting any field: its `**Unit scope:**` must match this run's batch. A manifest naming other Issues is treated as absent — rewrite it from this run's batch before trusting any field.
...
1. Re-read the status with `bees show-ticket --ids <issue-id>` and run `bees update-ticket --status done` only if it is not already `done`.
