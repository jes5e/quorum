...
When the Analyst's completion notification fires, read the Analyst's return — the structured Design Proposal per the contract in `agents/analyst.md` `## Structured-output contract`. **Mark that Analyst's `analyst-<issue-id>` (or `-rev<n>`) TaskList task `completed` the moment its return is read, before surfacing the proposal and before creating the gate task below** — its Agent has exited, so leaving it active would strand a task no completion notification will ever clear, exactly as for every other Agent task. From that moment the pending decision is carried by the `gate-askuserquestion-*` task (its `metadata.activity` holding the finite choices) while the gate is open, and by the conversation once a branch is entered — which is exactly the state Section 4's Read-state step re-derives when a compaction takes it. The proposal is **free-form analysis** (Problem / Root cause / Recommended approach / Blast radius / Policy decisions this change implies / Why / Alternatives considered / Options the body did not consider / Upstream-fetch status / verdict trailer); surface it to the user as prose, NOT as an `AskUserQuestion` body — `AskUserQuestion` is for finite-multi-choice gates, not free-text design recommendations. **Unlike `### Deferred refinements`, the `### Blast radius` and `### Policy decisions this change implies` sections ARE surfaced** — they are user-facing design content that the Approve decision covers (the enumerated sites the fix is expected to touch, and the yes/no policy answers the user ratifies), not orchestrator plumbing; surface both in full, including their fixed empty lines (`No invariant added, removed, or weakened.` / `None — the recommendation leaves no policy question open.`) when the Analyst emitted those, so the user can tell "swept and found nothing" from "never swept". The Analyst's `### Deferred refinements` block (per `agents/analyst.md`'s structured-return shape) is **consumed by the orchestrator and NOT surfaced to the user** — it is orchestrator plumbing for Section 7.5's deferral-hygiene gate, not user-facing design content. The consumption paragraph below ("Consume the Analyst's `### Deferred refinements` block") is the load-bearing site for that consumption; strip the `### Deferred refinements` block from the surfaced proposal before rendering the user-visible prose.
...
**The preamble also carries the policy decisions and any scope-split recommendation.** When the Analyst's `### Policy decisions this change implies` section is non-empty (anything other than its fixed `None — the recommendation leaves no policy question open.` line), the preamble MUST name **how many** decisions it carries and point at the section, so the user knows Approve ratifies the Analyst's recommended answers along with the design — the questions and the answers themselves ride the section, surfaced in full immediately below. Likewise, when the Analyst's `### Blast radius` carries a **scope-split recommendation** (fold the distinct concern into this Issue, or file it separately and only document the property here), the preamble says one is carried and points at that section. Both ride this existing preamble paragraph and the existing Approve / Revise / Cancel gate below — do **not** add a second gate, an extra option, or a separate task for them.
...
- Options:
  - **Approve & proceed to implementation (Recommended)** — Accept the Analyst's Recommended approach as the authoritative design directive for this Issue. Approval covers the design, the Analyst's recommended answer to every question in `### Policy decisions this change implies`, and any scope-split recommendation carried in `### Blast radius`. The orchestrator carries the directive forward into Section 4's implementer dispatch (the Engineer dispatch prompt when source code needs modification, the Test Writer and Doc Writer dispatches otherwise per Section 4's conditional-dispatch rules).
...
- **Approve** — Capture the Analyst's Design Proposal as the run's authoritative design directive for this Issue (specifically: the `### Recommended approach`, `### Blast radius`, and `### Policy decisions this change implies` sections — all three are directive, not context — plus the `### Why` / `### Alternatives considered` / `### Options the body did not consider` context the Engineer may want to consult). The corresponding TaskList task `analyst-<issue-id>` was already marked `completed` when its return was read (per the surfacing step above), so there is nothing to close out here. Pass the directive forward to Section 4's Engineer dispatch prompt per "Authoritative design directive" below. Proceed to Section 4.
...
- **Revise** — Continue the conversation with the user in prose. The user typically supplies feedback in the form "I disagree with X because…" or "have you considered Y?" — engage with that feedback in the conversation thread. When the user's feedback warrants a fresh codebase pass (a substantively different approach is on the table, the user wants the Analyst to investigate a specific code path, etc.), **re-dispatch the Analyst** as a fresh cold invocation:
  ...
  - The prior Analyst's TaskList task — `analyst-<issue-id>` on the first revision, or `analyst-<issue-id>-rev<n-1>` on subsequent revisions — was already marked `completed` when its return was read (per the surfacing step above), so there is nothing to close out before dispatching.
  ...
  - The new dispatch prompt embeds the same Issue body verbatim and `reference_materials` as the first dispatch, PLUS a clearly-labeled `## Prior proposal and user feedback` section that quotes the prior Design Proposal in full and the user's revision feedback verbatim. **"In full" includes the prior proposal's `### Deferred refinements` block**, which the surfacing step above strips before rendering the user-facing prose but which the revising Analyst needs in hand so it either carries each prior deferral forward or explicitly drops it — the supersede-and-clear rule below clears the superseded iteration's `defer-*` tasks, so a deferral omitted from the revision is dropped with no carrier.
...
- The Analyst's `### Blast radius` and `### Policy decisions this change implies` sections verbatim. These sit on the **directive** side alongside the Recommended approach, not the context side: the first is the enumerated site list the fix is expected to cover, and the second carries the policy answers the user ratified at the gate.
...
1. **Read state.** Pull the current truth from four sources before deciding what to do:
   ...
   **Conversation memory is never a substitute for these four sources.** Whatever the orchestrator believes it remembers about Issue status, landed commits, the batch order, or the pre-session SHA, the four sources above are the truth and are re-read rather than recalled. This is unconditional — it holds on every tick, not only after something goes wrong. On top of it, one conditional rule for the observable case: **if a summarization marker is visible in the conversation** (the harness compacted mid-run), treat everything before it as non-authoritative and re-read all four sources in full before dispatching anything else — starting with re-reading the Issue body from bees rather than working from a summary of it. One value has no durable carrier by design: Section 3's **Design Proposal** (approved, still pending at Section 3's gate, or not yet produced by a stranded first pass) — the `### Recommended approach`, `### Blast radius`, and `### Policy decisions this change implies` sections that compose the authoritative design directive, plus the context captured with them — lives only in the Analyst's returned message. So the rule that recovers it is **functional, not positional**: **whenever the proposal is needed for this Issue** — an Engineer dispatch (any round, including a Phase C re-entry round), a Phase A Code Reviewer relay, a Phase C PM relay, or a re-run of any of these, **or Section 3's own gate for this Issue is open on a proposal no reader can see** — **and it is no longer readable** (a compaction landed since the Analyst was dispatched, whether or not it returned first), do **not** reconstruct it from a summary, and do **not** give it a manifest field. **Re-derive it through the Analyst. That is the only recovery *for the proposal itself*** (the branch below that waits on an Analyst already in flight is that same re-derivation, already running) — there is no path in fix mode that omits a downstream `## Blast radius` relay heading, because the proposal is re-derived before the dispatch that needs it and is therefore in hand at every dispatch that relays it.

   **Check first for an active `analyst-<issue-id>` task for this Issue**, matching on the name **prefix** plus status so a `-rev<n>`-discriminated name is caught alongside the undiscriminated one. An active task means an Analyst is **genuinely in flight** — Section 3 marks the task `completed` the moment the Analyst's return is read, so an active one still has an Agent behind it (a first Section 3 pass, or a design-question revision per the Reconcile step's design-question rung below). There is nothing to re-dispatch, and that return **is** the recovery: **wait for it** and run Section 3's gate on it as normal. **Except when the active task is `pending` with no Agent behind it.** A `pending` `analyst-*` task for which **no dispatch has landed** will never produce a completion notification, and there are no clock primitives to time it out (see "Anti-pattern: no clock primitives" below), so waiting on it deadlocks the run — exactly as the Engineer-dispatch precondition's stranded-`pending` clause below describes. Therefore: wait only when that task is `in_progress`, or is `pending` **with a dispatch already landed**; when it is `pending` and **no dispatch has landed for it**, do not wait — dispatch the Analyst now under that same task name, using the **re-derivation shape** the bullets below define, and wait on that return instead. **Post-compaction, whether the dispatch landed is itself unreadable**, and the answer is the not-landed default that same clause states: treat the dispatch as **not** landed and re-dispatch. **Otherwise, when a `gate-askuserquestion-*` task for this Issue's Section 3 gate is active** — the gate fired and no answer has come back — the compaction took the proposal that gate asks about (its `metadata.activity` carries the finite choices, not the proposal they approve): mark that gate task `completed`, recording in its `metadata.activity` that a compaction abandoned the question — informational context, never a routing input — and re-dispatch the Analyst under the next `analyst-<issue-id>-rev<n>` name using the **re-derivation shape** the bullets below define. **For this one gate that re-derivation takes precedence over the generic gate re-fire** the TaskList naming convention below prescribes for an active `gate-*` task, because re-firing a gate whose proposal no reader can see asks the user to approve nothing. **Otherwise** — neither an `analyst-*` task nor this Issue's Section 3 `gate-*` task active — re-dispatch the Analyst under the next `analyst-<issue-id>-rev<n>` name using the **re-derivation shape**, defined once here and referenced from the design-question rung below:

   - the **Issue body re-read from bees**, and the same `reference_materials` the first dispatch carried;
   - the **changed-file list of what is on disk for this Issue** — the latest Engineer return's `## Files changed` list, or, when no return carries one, the working-tree derivation the Test Writer dispatch's **Fallback when an Engineer return did not list files** below already gives as a paired POSIX / PowerShell command — an **empty** list is itself informative, saying no implementation has landed for this Issue yet;
   - a plain statement that **the prior proposal was lost to a compaction** and that the changed-file list above is what has already been implemented against it, so the Analyst **re-derives** the design from the codebase rather than revising a summary of one, and its `### Blast radius` must cover the tree as it stands. When the pass being re-dispatched is the **first** Section 3 pass for this Issue — the stranded task carries the undiscriminated `analyst-<issue-id>` name, so no proposal was ever produced — say instead that **no proposal has been produced yet** and that the compaction landed before one was returned; the rest of this bullet is unchanged.

   When that Analyst returns, run Section 3's surface-and-gate flow unchanged and resume the phase that was in progress, now carrying the re-derived directive — **branching only on TaskList-derivable state**, never on whether Phase A "still had findings", which is precisely the value the Reconcile step's **Post-compaction recovery inside Phase A** rung declares has no durable carrier. So: when **Phase A has not begun** — no `engineer-<issue-id>` task exists for this Issue, matching on the name **prefix** so a `-r<n>`-discriminated name is caught alongside the undiscriminated one, **or the only one that does is `pending` with no dispatch landed for it** (the task is created before the `Agent(...)` call lands, so a compaction in that window strands one with no Agent and no diff behind it; whether the dispatch landed is unreadable post-compaction, and the not-landed default the Engineer-dispatch precondition's stranded-`pending` clause below states governs here, as it does at the Analyst check above) — resume at **Phase A's entry**: the first Engineer dispatch, made under that stranded task's own name when one exists rather than a fresh discriminated one, exactly as the Analyst check above prescribes for its own stranded `pending` task; or straight to **Phase B** when the re-derived directive needs no source change. Do **not** dispatch the Code Reviewer on that branch; no implementation has landed for it to review, which is the same emptiness the changed-file list bullet above reports. **The "only one that does" qualifier is deliberate:** when a stranded `pending` `engineer-<issue-id>-r<n>` sits alongside an already-`completed` earlier round, this arm does not fire, the arms below govern, and the stranded task is left exactly as it is — Section 7's close-out prefix sweep marks it `completed` at the Issue boundary, so nothing leaks past that boundary and the only cost is a skipped round number. Otherwise, when an `engineer-<issue-id>` task does exist and **Phase B has not begun** — no `test-writer-<issue-id>` or `doc-writer-<issue-id>` task exists for this Issue, prefix-matched the same way — resume through that rung, which dispatches the **Code Reviewer** with the re-derived `## Blast radius` relayed to it (that rung's Analyst-active exception cannot fire at this point — Section 3's surfacing step marked this Issue's Analyst task `completed` the moment its return was read), and let the Code Reviewer's findings drive whether an Engineer round follows and what that round addresses (under its next name per the naming convention below). Otherwise resume with the **next dispatch** in the ladder — whose `## Blast radius` relay now has a list to carry. **One case the Phase-B-has-not-begun arm cannot tell apart by task-name set alone:** a round whose prior Engineer return carried a `## Design question`, where the compaction landed before the design-question rung's Analyst re-dispatch — that rung holds Phase A suspended rather than reviewing the partial diff, and nothing in the task-name set records that it was reached. What this costs is a Code Reviewer round spent on a partial diff, not a mis-completion: its findings are additive, and the Engineer round that follows carries the re-derived directive. The cost is real and worth stating plainly: the operator is asked to approve a proposal it was already shown once, and where the re-derivation differs from the diff already on disk, that difference is information the Engineer round reconciles rather than a contradiction to paper over.
...
2. **Reconcile.** Compare current state to target state and act. Dispatch for an Issue is **ordered into three phases** — the implementer lanes are NOT fanned out together, and the reviewers are not all held to the end. Walk the phases in order; dispatch concurrently only *within* a phase.
   ...
   - **Phase A — source to clean.** When the fix needs source changes, dispatch the **Engineer alone** (see "Per-issue cold dispatch" below). On its return, dispatch the **Code Reviewer alone** (Section 5 carries the reviewer dispatch shape and `code-reviewer-<issue-id>` naming) — **unless that return carries a `## Design question`**, which the Reconcile step's design-question rung below routes back into Section 3 instead. Loop Engineer → Code Reviewer → Engineer until either the Code Reviewer emits its clean shape (`No code issues found.`) or every remaining finding has been routed through `### Orchestrator discipline: routing review findings` — accepted into the session-scoped compromise tracker (Section 7.5), recorded as a `defer-*` task (Section 5's ignored-feedback rule), or gated to the user. That design-question rung is a **third way out of the Engineer step** alongside those two exit conditions: it neither closes Phase A nor continues the loop, but **suspends** Phase A pending a revised directive from Section 3. Every round after the first uses a round-discriminated TaskList name (`engineer-<issue-id>-r<n>` / `code-reviewer-<issue-id>-r<n>`, per the TaskList naming convention below). When the Issue needs no source change at all, Phase A is empty and the loop proceeds directly to Phase B.
   ...
   - **Engineer-dispatch precondition (checkable, not a promise).** The orchestrator **MUST NOT dispatch an Engineer Agent for this Issue while any TaskList task whose name begins with `test-writer-<issue-id>`, `doc-writer-<issue-id>`, `test-reviewer-<issue-id>`, `doc-reviewer-<issue-id>`, `pm-<issue-id>`, or `analyst-<issue-id>` is `pending` or `in_progress`.** Match on the name **prefix** plus the status, never on an exact name — Phase A's round discriminator (`-r<n>`) and any other suffix a re-dispatch appends must still be caught by this check. This is a **precondition to check, not a state to remember**: before *every* Engineer dispatch — forward Phase A rounds and re-entry rounds alike — walk the TaskList (one of the four authoritative read-state sources this tick already reads) and confirm no task matching those prefixes is active. Deriving the rule from the TaskList rather than from orchestrator memory is what makes it survive a compaction — see the "Conversation memory is never a substitute for these four sources" discipline above.
     ...
     **Why the Analyst is covered too.** An Analyst mid-revision — the design-question rung below, or Section 3's own Revise loop — is authoring the very directive the next Engineer round would implement, so an Engineer dispatched alongside it runs against the stale directive that just failed, which is the checkable form of the Phase A suspension the design-question rung asserts in prose. Adding the prefix cannot block the forward path: Section 3 marks the Analyst's task `completed` the moment its return is read, before the approval gate fires, so no active `analyst-*` task exists at the Approve → Engineer dispatch.

     **The Code Reviewer is the one deliberate exception, and is NOT in the prefix list.** Phase A alternates Engineer and Code Reviewer by construction — the Code Reviewer is dispatched only on an Engineer's return, and the next Engineer is dispatched only on the Code Reviewer's return — or, on the design-question rung's path, on Section 3's Approve of the revised proposal, with no Code Reviewer dispatched for that round at all — so a `code-reviewer-<issue-id>` task is never in flight at the moment an Engineer dispatch is being considered. Listing it would add a test that can never fire, and would read as licence to dispatch the two concurrently if it ever did. The exclusion rests on that alternation, so it is specific to this ladder: it does **not** generalize to a review site where the Code Reviewer runs concurrently with other reviewers.

     **When the blocking task is `pending` with no Agent behind it.** `pending` is in the status test deliberately: it closes the race where the orchestrator has created the task but the `Agent(...)` call has not landed yet. But a `pending` task with no dispatch behind it will never produce a completion notification, and there are no clock primitives to time it out (see "Anti-pattern: no clock primitives" below), so waiting on it deadlocks the loop. Therefore: when the blocking task is `in_progress`, or is `pending` **with a dispatch already landed**, wait for its completion notification and re-check on the next tick. When it is `pending` and **no dispatch has landed for it**, do not wait — either dispatch that role now (when its work is still wanted) or mark the stale task `completed` and clear it from the active set (when it is not), then re-check the precondition. **Post-compaction, whether a dispatch landed is unreadable** — the `Agent(...)` call lived in the conversation the compaction destroyed, and the TaskList records nothing about it — so default to **not** landed: a duplicate dispatch costs one pass, while a dispatch wrongly assumed to have landed deadlocks the run on a notification that never comes. That default is stated here once and is the answer wherever this clause is cited. Do **not** narrow the rule to `in_progress` only; `pending` stays in the test, and this clause is what keeps it from stranding the loop.
   ...
   - **Post-compaction recovery inside Phase A.** "Did the last code review come back clean?" has **no durable carrier**, and it should not be given one. The run-state manifest records only values that live nowhere else on disk; a lane's phase is not such a value, because it is **re-derivable at any moment by asking the Code Reviewer again**. So do NOT add a lane-phase field to the manifest. If a compaction lands mid-Phase-A and the orchestrator cannot read the Code Reviewer's last verdict from the conversation, **re-dispatch the Code Reviewer** — unless an `analyst-<issue-id>` task is active for this Issue (a design-question revision in flight, per the design-question rung below), in which case Phase A is suspended pending the revised directive: wait for that Analyst's return and run Section 3's gate on it rather than reviewing the partial diff. Match on the `analyst-<issue-id>` name **prefix** plus status, the same prefix-match discipline the Engineer-dispatch precondition uses, so a `-rev<n>`-discriminated name is caught alongside the undiscriminated one — this exception is the Phase-A-local instance of the Analyst-active check the Read-state step's re-derivation rule makes first. Otherwise a cold fresh-eyes pass over the current diff is idempotent and cheap, and is the correct recovery rather than guessing the prior verdict.
   ...
   - For every Agent that has reported completion, persist the result: confirm any bees ticket transitions the Agent committed to, mark the corresponding TaskList task `completed`, and unlock the next phase (or the next lane within the current phase) per the ladder above. **A Phase B writer return that reports detected source movement is not a completion** — route it through the movement-report rung above instead: it does not unlock the next phase. That rung does close out the writer's own TaskList task (the Agent has exited), but it opens an `aborted-*` task in its place, and Phase C stays shut while that task is `pending`.
     ...
     **An Engineer return carrying a `## Design question` is not a Phase A completion either.** `agents/engineer.md` requires the Engineer to stop rather than invent a mechanism the approved directive did not enumerate, and to return the open question instead — so the diff on disk is partial by construction. Such a return is evidence that the approved proposal's `### Blast radius` **missed a mechanism**, which makes it a revision of that proposal rather than a new decision point here. Mark the Engineer's own TaskList task `completed` (its Agent has exited) and do **not** dispatch the Code Reviewer against that partial diff. Then route the question as a **Section 3 Revise, by reference**:

     - **Re-dispatch the Analyst exactly as Section 3's Revise branch prescribes** — under the next `analyst-<issue-id>-rev<n>` name, with the same Issue body verbatim and the same `reference_materials` as the first dispatch, and a `## Prior proposal and user feedback` section that quotes the approved Design Proposal in full — **"in full" includes its `### Deferred refinements` block**, which Section 3 strips before rendering the user-facing prose but which the revising Analyst needs in hand so it either carries each prior deferral forward or explicitly drops it, since Section 3's supersede-and-clear rule on Approve clears the superseded iteration's `defer-*` tasks and a deferral omitted from the revision would otherwise be dropped with no carrier — and, **in place of the user's revision feedback**, the Engineer's `## Design question` verbatim (the mechanism it would have had to add, why the assignment cannot be completed without it, the alternatives it named, and which parts of the assignment landed and which did not). The re-dispatch prompt MUST additionally state that a **partial implementation of the prior directive is already on disk** — naming the Engineer's `## Files changed` list — so the Analyst reads the working tree as **mid-change** rather than as the pre-fix baseline, and MUST state that its revised `### Blast radius` has to cover the lifecycle of whatever mechanism it now enumerates.

       **When the approved Design Proposal is no longer readable**, use the **re-derivation shape** the Read-state step above defines — it is the single definition of what to send in place of a proposal a compaction stranded, and it already carries the Issue body re-read from bees, the `reference_materials`, the changed-file list of what is on disk, and the statement that the prior proposal was lost to a compaction. This rung adds one thing to it: the Engineer's `## Design question` travels in the `## Prior proposal and user feedback` section as the feedback. Everything else about the re-dispatch is unchanged.
     - **When the Analyst returns, run Section 3's surface-and-gate flow unchanged.** The revised proposal is surfaced with its verdict preamble; Section 3's gate fires through the two-step contract with its three options exactly as that section defines them; and the branches are Section 3's own — **Approve** captures the revised directive (its `### Deferred refinements` supersede-and-clear rule applies as written, because this is a fresh Analyst return), **Revise** loops, and **Cancel** routes through Section 7's **`#### Aborted-Issue close-out`**, which already names the uncommitted working-tree state this partial diff leaves behind. After Approve, re-dispatch the Engineer under its next `-r<n>` round-discriminated name per the naming convention below, carrying the revised directive and a note that it continues from the partial diff on disk.
     - **The durable carrier is the Analyst task.** While that Analyst runs, the active `analyst-<issue-id>-rev<n>` task is what holds Phase A suspended — the same carrier Section 3's Revise loop already relies on; on its return that task is marked `completed` per Section 3's surfacing step, and Section 3's gate task carries the suspension from there until a branch is entered. Nothing about the question lives only in conversation **once that re-dispatch lands**: it is embedded in the Analyst's dispatch prompt and comes back inside the revised proposal. The residual is the window before it — a compaction landing between the Engineer's return and that re-dispatch leaves no trace of this rung in the task-name set, which costs a Code Reviewer round spent on the partial diff rather than a mis-completion, exactly as the Read-state step's resume arms above record.

     This receiver introduces no gate, option, task-name class, marker, or manifest field of its own — it is Section 3's Revise branch, entered from Section 4.
...
**Engineer dispatch — embed the Section 3 design directive as the authoritative design source.** When the dispatched role is `engineer`, the dispatch prompt MUST additionally embed the design directive captured at the end of Section 3's Approve branch (the Analyst's `### Recommended approach`, `### Blast radius`, and `### Policy decisions this change implies` sections, plus `### Why`, `### Alternatives considered`, and `### Options the body did not consider` for context, plus any user-supplied refinement captured on the Approve branch). Label that block clearly in the prompt — recommended labels are `## Authoritative design directive (from Section 3 Analyst pass)` for the three directive sections and `## Design context (Analyst's Why / Alternatives considered / Options the body did not consider)` for the surrounding context — so the Engineer can distinguish the directive from the verbatim Issue body. **Relay the `### Blast radius` section under the heading `## Blast radius`** inside that directive block: it is the **enumerated site list** `agents/engineer.md` reconciles its own sweep against ("When the dispatch prompt carries an enumerated site list … account for **every** entry"), and the policy section carries the answers the user ratified, so the Engineer implements them rather than re-deciding them mid-fix. **State in the prompt that the directive is the enumeration**: a mechanism the directive does not name is a design question the Engineer stops on and returns under a `## Design question` heading per `agents/engineer.md`, not a choice the Engineer makes — the Reconcile step's design-question rung above routes such a return back into Section 3 as a Revise, re-dispatching the Analyst so the revised proposal enumerates it. Where the Issue body's framing and the design directive conflict, **the directive wins**: the body still flows in verbatim because identifier spellings, contract surfaces, and other byte-accurate references travel with it, but the design source the Engineer implements is the directive. The Engineer's role file (`agents/engineer.md`) carries the same invariant in its **Authoritative design directive (fix mode only)** bullet, so the directive-wins rule is reloaded on every cold dispatch even if the dispatch-prompt framing is terse. The Test Writer and Doc Writer dispatch prompts do NOT carry the design directive separately — they read the resulting diff (the Engineer's implementation of the directive) as their work signal, per the hub-and-spoke flow.

**Code Reviewer dispatch — relay the Engineer's completeness evidence verbatim.** `agents/engineer.md` requires the Engineer's return to carry a **completeness check with evidence** whenever its assignment directed a change at *every site* where some property holds (the search patterns run, every hit, and per hit either the change made or the reason it is deliberately untouched). That evidence has no other carrier — the Engineer Agent has exited and its return lives only in this conversation — so **when an Engineer's return for this Issue carried a completeness list, the Phase A Code Reviewer dispatch prompt MUST embed that list verbatim** under a clearly labelled heading; the recommended label is `## Engineer's completeness evidence`. Embed it as text in the prompt (the same way the Issue body and the design directive are embedded) — do **not** write it to a scratch file and pass a path. `agents/code-reviewer.md` carries the Code-Reviewer-side half (pass the list through into its `/quo-engineer-review` invocation). **Separately from the list, state in the prompt that the assignment was sweep-shaped whenever it was** — that is a fact about the assignment that was dispatched (a directive or Subtask body that directed a change at *every site* where some property holds), known independently of whether a list came back. `/quo-engineer-review`'s sweep-verification check only reports a *missing* list as a finding when the invocation itself named the assignment that way, so an unnamed sweep silently loses the check — and the case that most needs the check is precisely a sweep-shaped assignment whose Engineer returned no list. When no completeness list arrived, omit the `## Engineer's completeness evidence` heading rather than emitting an empty one — but still state that the assignment was sweep-shaped when it was, so `/quo-engineer-review`'s missing-list check stays reachable. **Unlike the `### Blast radius` relay below, this MUST is not satisfiable on every path:** the Engineer's list lives only in this conversation, and nothing re-derives it, so a compaction landing after the Engineer returned destroys it. On that resume the negative branch just stated is what governs — omit the `## Engineer's completeness evidence` heading, and state the sweep-shaped fact, which is still known from the assignment itself. **State the cost plainly rather than dressing it up:** on that dispatch `/quo-engineer-review` will report the completeness list as missing, and no wording of this prompt prevents that — its missing-list check fires on the sweep-shaped statement, and independently on the non-empty `## Blast radius` the relay below makes a MUST here, which arrives with no Engineer reconciliation of it. That finding is a **compaction artifact, not an Engineer defect**, and the orchestrator dispositions it as one: rather than auto-dispatching the Engineer per part (a)'s table in `### Orchestrator discipline: routing review findings`, it records the finding as ignored feedback per **Section 5's ignored-feedback rule** (record the ignored item as a `defer-<short-suffix>` TaskList task at the moment of the ignore decision, annotated `addressed-now-in-this-Issue` — the fix-mode spelling that rule gives, and the only one of its three destination labels that fits, since neither a new Issue nor an existing ticket body can carry a list nothing reproduces; leave the task `pending` so Section 7.5's deferral-hygiene gate still surfaces it, and list it in the per-Issue summary's `**Ignored Review Feedback**` field) — the closure path Phase A already borrows and the Phase A exit test already counts — because no Engineer round can reproduce a list the compaction already destroyed.

**Code Reviewer dispatch — relay the Analyst's `### Blast radius` verbatim.** Alongside the completeness evidence, when Section 3's approved proposal carried a `### Blast radius` section, the Phase A Code Reviewer dispatch prompt MUST embed it **verbatim** under the heading `## Blast radius`. Use that one heading string at every hop — the same single-string discipline the completeness-evidence chain uses — because the review verifies the diff against the list under exactly that label, and a renamed heading silently drops the check. Embed it as text in the prompt, the same way the Issue body and the design directive are embedded; do **not** write it to a scratch file and pass a path. **Relay it even when the Analyst's section carried only its fixed empty line** (`No invariant added, removed, or weakened.`) — the review's sweep-verification check needs to see "nothing to verify" rather than "not supplied", and those two are indistinguishable once the heading is omitted. This MUST is satisfiable on every path: when a compaction stranded the proposal, the Read-state step above re-derives it through the Analyst *before* this dispatch, so there is always a section in hand to relay. `agents/code-reviewer.md` carries the Code-Reviewer-side half (forward the block into its `/quo-engineer-review` invocation under the same heading).
...
- **Analyst Agents** (dispatched per Issue in Section 3 before the implementer phase) — Name: `analyst-<issue-id>` (e.g., `analyst-veq` for Issue `b.veq`). When the user picks `Revise` and the orchestrator re-dispatches the Analyst with the user's feedback as additional context, append a `-rev<n>` discriminator where `<n>` is the 1-based revision count (e.g., `analyst-veq-rev1` for the first revision, `analyst-veq-rev2` for the second) — per Section 3's Revise-branch instructions. The same `-rev<n>` discriminator is used when Section 4's Reconcile-step **design-question rung** re-dispatches the Analyst, since that rung is Section 3's Revise branch entered from Section 4.
...
**The Phase C PM dispatch prompt must also relay the Engineer's completeness evidence verbatim.** Same rule as the Phase A Code Reviewer dispatch (see "Code Reviewer dispatch — relay the Engineer's completeness evidence verbatim" in Section 4), applied again here because the PM is a **second** `/quo-engineer-review` caller on this Issue: `agents/pm.md` drives that skill in flight via the `Skill` tool, and its PM-side bullet requires it to pass a completeness list through into the invocation — a list it can only pass on if this dispatch prompt gave it one. Relaying to the Phase A Code Reviewer does not cover this: those are different Agents with no channel between them, and the Phase C PM reviews a **wider** diff than the Phase A Code Reviewer ever saw (it includes the Phase B test and doc changes). So **when any Engineer return for this Issue carried a completeness list** (the search patterns run, every hit, and per hit either the change made or the reason it is deliberately untouched), embed it **verbatim** in the PM dispatch prompt under a clearly labelled heading — the recommended label is `## Engineer's completeness evidence`. Embed it as text in the prompt, the same way the Issue body is embedded; do **not** write it to a scratch file and pass a path. Attribute each list to its Phase A round when more than one Engineer round produced one. **Separately from the list, state in the prompt that the assignment was sweep-shaped whenever it was** — that is a fact about the assignment that was dispatched (a directive or Subtask body that directed a change at *every site* where some property holds), known independently of whether a list came back. `/quo-engineer-review`'s sweep-verification check only reports a *missing* list as a finding when the invocation itself named the assignment that way, so an unnamed sweep silently loses the check — and the case that most needs the check is precisely a sweep-shaped assignment whose Engineer returned no list. When no completeness list arrived, omit the `## Engineer's completeness evidence` heading rather than emitting an empty one — but still state that the assignment was sweep-shaped when it was, so `/quo-engineer-review`'s missing-list check stays reachable. **This MUST is not satisfiable on every path**, for the same reason it is not at the Code Reviewer dispatch: a compaction landing after an Engineer returned destroys that round's list. **Apply that per round, not per Issue:** embed every list still in hand, each attributed to its Phase A round, and let a round whose list a compaction destroyed contribute nothing. A compaction mid-Phase-A takes the earlier rounds' lists, while the rounds dispatched after the recovery return lists that *are* in hand at this dispatch — discarding those too would throw away evidence the review can still verify against. So omit the `## Engineer's completeness evidence` heading only when **no** list survives. Whenever a list is missing for that reason, any missing-list finding the PM's `/quo-engineer-review` returns takes the same compaction-artifact disposition stated at the Code Reviewer dispatch above.

**The Phase C PM dispatch prompt must also relay the Analyst's `### Blast radius` verbatim.** Same rule as the Phase A Code Reviewer dispatch (see "Code Reviewer dispatch — relay the Analyst's `### Blast radius` verbatim" in Section 4), applied again here for the same reason the completeness evidence is relayed twice: the PM is a **second** `/quo-engineer-review` caller with no channel to the Phase A Code Reviewer, and it reviews a wider diff. So when Section 3's approved proposal carried a `### Blast radius` section, embed it **verbatim** in the PM dispatch prompt under the heading `## Blast radius` — one heading string at every hop, as text in the prompt, never as a scratch-file path. **Relay it even when the Analyst's section carried only its fixed empty line** (`No invariant added, removed, or weakened.`) — the sweep-verification check needs to see "nothing to verify" rather than "not supplied". This MUST is satisfiable on every path for the same reason it is at the Code Reviewer dispatch: a compaction that stranded the proposal is recovered by the Section 4 Read-state step's re-derivation before this dispatch runs. `agents/pm.md` carries the PM-side half (forward the block into its `/quo-engineer-review` invocation under the same heading).
...
3. Mark the per-issue TaskList tasks as `completed` and clear them from the active set. All of them are named per Section 4's issue-scoped naming convention; group them by the phase that dispatched them:
   ...
   - the Section 3 Analyst task `analyst-<issue-id>` (plus any `analyst-<issue-id>-rev<n>` revisions) — already `completed` on return, so the sweep is a no-op for it;
...
This is the canonical anchor name for the path where an Issue ends **without a fix landing**. Two branches route here: Section 4's Reconcile-step unexplained-movement gate's **Abort this Issue** choice, and Section 3's Analyst-proposal gate's **Cancel** choice. Like the two definitions above it, **this is a definition, not a step in Section 7's linear flow** — do not run it merely because reading reached this heading; run it only where one of those branches calls for it by name.

An aborted Issue is still an Issue boundary. Leaving one by simply "moving on" skips every close-out the fixed path runs — the prefix-swept TaskList close-out, the deferral-hygiene gate, and the state-externalization checkpoint — and in batch mode this boundary ends the run rather than advancing to the next Issue (step 4). That is how an `aborted-*` marker survives into a later re-entry and holds Phase C shut on an Issue that has nothing wrong with it, how a `defer-*` entry is stranded with no durable carrier, and how the manifest is left naming an Issue this run has already left. Run these four steps in order:

1. **Close out this Issue's TaskList tasks**, exactly the sweep Section 7 step 3 runs on the fixed path: sweep by name **prefix**, not by exact name, and mark `completed` every task scoped to this Issue — the Section 3 `analyst-<issue-id>` task (plus any `-rev<n>` revisions), already `completed` on return so the sweep is a no-op for it, the Phase A `engineer-<issue-id>` / `code-reviewer-<issue-id>` tasks, the Phase B `test-writer-<issue-id>` / `doc-writer-<issue-id>` tasks, the Phase C `test-reviewer-<issue-id>` / `doc-reviewer-<issue-id>` / `pm-<issue-id>` tasks, every `-r<n>`-discriminated round of any of them, and the `gate-*` task that fired the branch routing here. **Include every `aborted-<role>-<issue-id>` redelivery marker still `pending`** — on the Abort-this-Issue branch that marker is precisely why this close-out is being run, and one left `pending` past the boundary holds Phase C shut on any later re-entry into this Issue. Mark each `completed` with the abort reason recorded in `metadata.activity`: informational context for whoever re-opens the Issue, never a routing input. Throughout, **"clear from the active set" means mark the task `completed`** — never delete it, or the `-r<n>` round count the naming convention derives by counting recorded rounds goes with it.

   **A sibling lane still in flight when the abort fires.** Phase B dispatches the Test Writer and Doc Writer concurrently, so the lane that did not abort may still have a live Agent behind it, and the orchestrator cannot terminate a dispatched background Agent. Mark its task `completed` anyway and record in `metadata.activity` that it was in flight at the abort. Its completion notification, when it arrives, is a tick for an Issue this run has already closed out: process nothing for that Issue on it, note that the lane's work may be partially landed in the tree, and continue with whatever unit the run is on.
2. **Run Section 7.5's deferral-hygiene gate** for this Issue. Deferrals recorded before the abort — the bullets consumed from the Analyst's `### Deferred refinements` block, any reviewer or PM feedback the Director chose to ignore during Phase A — are exactly as stranded on an aborted Issue as on a fixed one, and this gate is what moves each into a durable carrier. The gate's hard-stop applies unchanged: do not advance past it while any `defer-*` task is `pending` or `in_progress`.
...
4. **Read the working tree, then branch on mode.** This step is where the aborted path **diverges** from Section 7 step 5: an aborted Issue lands no commit, so the Engineer's edits — and any Phase B writer's partial work — stay uncommitted in the tree, and the run does **not** carry that tree into another Issue.
