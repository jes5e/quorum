...
| `## Design decisions for writers` | the approved proposal's `### Decisions for writers`, verbatim | Test Writer, Doc Writer | relay `None — the approach leaves no test or doc decision to the writers.` when that is all the section carries; never omit while a proposal exists |
| `## Engineer's diff (path)` | the path of a file written just before the dispatch with `git diff <pre-session-sha> --output=<tempdir>/.quorum/diff-<issue-id>-<short-suffix>.md` — one literal command, identical on POSIX and PowerShell once the `.quorum` directory exists per Section 2 | Doc Writer | omit when Phase A was empty |
