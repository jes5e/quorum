...
1. **Read state.** Pull the current truth from four sources before deciding what to do:
   ...
   **Check first for an active `analyst-<issue-id>` task for this Issue**, matching on the name **prefix** plus status so a `-rev<n>`-discriminated name is caught alongside the undiscriminated one. An active task means an Analyst is **genuinely in flight** — Section 3 marks the task `completed` the moment the Analyst's return is read, so an active one still has an Agent behind it (a first Section 3 pass, or a design-question revision per the Reconcile step's design-question rung below). There is nothing to re-dispatch, and that return **is** the recovery: **wait for it** and run Section 3's gate on it as normal. **Otherwise, when a `gate-askuserquestion-*` task for this Issue's Section 3 gate is active** — the gate fired and no answer has come back — the compaction took the proposal that gate asks about (its `metadata.activity` carries the finite choices, not the proposal they approve): mark that gate task `completed`, recording in its `metadata.activity` that a compaction abandoned the question — informational context, never a routing input — and re-dispatch the Analyst under the next `analyst-<issue-id>-rev<n>` name using the **re-derivation shape** the bullets below define. **For this one gate that re-derivation takes precedence over the generic gate re-fire** the TaskList naming convention below prescribes for an active `gate-*` task, because re-firing a gate whose proposal no reader can see asks the user to approve nothing. **Otherwise** — neither an `analyst-*` task nor this Issue's Section 3 `gate-*` task active — re-dispatch the Analyst under the next `analyst-<issue-id>-rev<n>` name using the **re-derivation shape**, defined once here and referenced from the design-question rung below:
   ...
   - a plain statement that **the prior proposal was lost to a compaction** and that the changed-file list above is what has already been implemented against it, so the Analyst **re-derives** the design from the codebase rather than revising a summary of one, and its `### Blast radius` must cover the tree as it stands.

   When that Analyst returns, run Section 3's surface-and-gate flow unchanged and resume the phase that was in progress, now carrying the re-derived directive: an **Engineer round** under its next name per the naming convention below when Phase A still had findings to address or Phase B had not begun, otherwise the **next dispatch** in the ladder — whose `## Blast radius` relay now has a list to carry. The cost is real and worth stating plainly: the operator is asked to approve a proposal it was already shown once, and where the re-derivation differs from the diff already on disk, that difference is information the Engineer round reconciles rather than a contradiction to paper over.
