...
**Severity rules the choice set — a `blocker` can be neither deferred nor accepted.** When the finding reaching this gate carries the `blocker` severity tag, the `Defer to follow-up Issue` and `Accept the limitation` choices below are **unreachable**: do not offer them, do not mark either Recommended, and do not route to them by any other path. A `blocker` gets exactly **two** choices here:

- **Fix properly now** — as below.
- **Re-dispatch the Analyst with this finding** — for a `blocker` that shows the **approved design directive itself is wrong** rather than the implementation of it. Reuse Section 3's Revise-branch re-dispatch shape: mark the prior Analyst task `completed`, issue a fresh cold `subagent_type=analyst` dispatch tracked as `analyst-<issue-id>-rev<n>`, and carry the finding **verbatim** as the revision context in place of the user's revision feedback. When that Analyst returns, Section 3's Approve / Revise / Cancel approval gate re-fires over the new proposal, exactly as on the Revise branch.

The three choices below are the choice set for `suggestion`- and `nit`-severity findings. Because a `blocker` never reaches the `Defer to follow-up Issue` or `Accept the limitation` branches, Section 7.5's **Trigger A** and **Trigger B** are unreachable for `blocker` findings — no tracker entry can record a deferred or accepted blocker.
...
4. **Read the working tree, then branch on mode.** This step is where the aborted path **diverges** from Section 7 step 5: an aborted Issue lands no commit, so whatever the phases it reached produced — the Engineer's edits, any Phase B writer's partial work — stays uncommitted in the tree, and the run does **not** carry that tree into another Issue. How much is there depends on the entering branch (a Section 3 `Cancel` fires before any implementer ran, so typically nothing), which is why the next paragraph reads the tree rather than asserting what is in it.
