...
**Reviews**: [Code review: X issues found/None needed | Test review: Y issues found/None needed | Docs review: Z issues found/None needed — each slot carrying its own `N nits applied without re-review` (or "count unavailable post-compaction") when any, attributed by review site where a slot has more than one]
