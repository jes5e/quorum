...
7. **Solo only: review and approve.**
   ...
   - Choices: **Approve** sets the SDD `ready`, and the report records any `blocker` approved over. **Revise** applies the remaining findings and the user's changes, then reviews again. **Cancel** puts the prior body and status back; an SDD this run created stays `drafted`. Mark **Revise** (Recommended) when a `blocker` is open.
