...
7. **Solo only: review and approve.**
   ...
   - Choices: **Approve** (set the SDD `ready`; a `blocker` approved over is recorded in the report), **Revise** (apply the remaining findings and the user's changes, then review again), **Cancel** (an SDD with a saved copy gets it back; otherwise it stays `drafted`).
