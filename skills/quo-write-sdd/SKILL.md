...
3. **(Optional) Findings to address.** A `findings:` block carrying a numbered list of severity-tagged work items the caller wants this skill to address on the revise pass. Used canonically by `/quo-plan`'s Step 4c spec-review revise loop (carrying `/quo-spec-review` findings) and `/quo-plan`'s Step 5e plan-review revise loop (carrying fresh-eyes plan-review findings); omitted on the initial SDD-authoring invocation. The field accepts **two interchangeable item shapes**, each preserving the verbatim severity tag and one-line description from the originating reviewer:

   - **Spec-review shape** — severity tag in square brackets followed by an SDD-section anchor, e.g., `1. [blocker] SDD ## Codebase exploration findings — generic "the routing layer" reference; cite the actual module path so the Engineer has a starting point.`.
   - **Plan-review shape** — backticked severity tag followed by a `target:` tag (in place of a section anchor, since plan-level findings often address the document as a whole rather than a named section), e.g., `1. \`blocker\` target: SDD — the cited module paths do not match the codebase's current package layout; re-run the codebase-research dispatch and re-ground the section.`.

   When this field is present, the skill routes the findings into Step 5's body re-authoring path (followed by Step 6's write-and-update path) as additional revision context (the same way Step 7a's revise loop on the solo path consumes them) — the load-bearing effect is that Step 5's authoring pass treats the listed findings as required fixes against the existing SDD body, including (where appropriate) re-dispatching Step 3's Explore-agent codebase research if a finding indicates the cited module names or fixture conventions need re-grounding. The two item shapes are functionally equivalent — the severity tag and descriptive body are what the re-authoring pass keys against; the anchor (section-name vs `target:` tag) is informational only. The field is OPTIONAL; absent or empty `findings:` means no findings to address (the normal initial-authoring shape). The shape mirrors `/quo-write-prd`'s `findings:` field exactly (both writers accept both item shapes) so `/quo-plan` can build per-writer payloads with the same template.
