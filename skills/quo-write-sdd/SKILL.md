...
- **One or more `blocker` items** — surface the full work-item list to the user via `AskUserQuestion` with finite choices:
  - **Revise** (recommended) — loop back to Step 5's body authoring with the findings, then re-run Step 6's write-and-update path and re-invoke `/quo-spec-review <spec-bee-id> --doc SDD` for a re-check.
