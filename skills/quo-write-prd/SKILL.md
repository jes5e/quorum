...
2. **Find the existing PRD.** Query the Spec Bee's children for one titled exactly `PRD`. None → you will create it. One → you will update it, so re-runs never duplicate it. On a solo run, when it is `ready`, first save it to `saved-<prd-id>-<short-suffix>.md` with its status on the first line and its body after. A cancelled solo revision restores the newest saved copy for this ID, so an approved PRD is never lost to a draft; the ID in the name lets a compacted or repeated run find it. More than one → stop and ask the user which is canonical.
...
6. **Solo only: review and approve.**
   ...
   - Choices: **Approve** (set the PRD `ready`; a `blocker` approved over is recorded in the report), **Revise** (apply the remaining findings and the user's changes, then review again), **Cancel** (a PRD with a saved copy gets it back; otherwise it stays `drafted`).
