...
- **Engineering best practices** (`docs/engineering-best-practices.md`) — coding standards, error handling conventions, module/package boundary rules, async/concurrency patterns, type design, API conventions, observability, storage patterns, security, code style, and dependency management. Ground every recommendation in the project's actual stack (e.g., for a Rust/tonic/tokio project: thiserror for errors, tracing for observability, dashmap for concurrency, clippy/fmt for style). Always include a comments section that states three rules in the project's own terms: anything machine-checkable goes into something that is machine-checked (a type, a named constant, an assertion with a message, a test whose name states the behavior) rather than into a comment, because a comment that restates code can be false with nothing failing; comments are reserved for what nothing can verify (external facts such as upstream bug numbers and contract terms, rejected alternatives and why they failed, invariants that span files); and comments that ship as documentation (protocol-schema and interface-definition comments, doc comments on public items a reference renders, endpoint descriptions) are contract text held to a completeness standard, not code comments, and are changed in the same edit as the behavior they describe.
