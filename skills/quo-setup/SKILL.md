...
- **`/quo-plan`** — interactive scope discovery for an idea, refactor, or feature without finalized specs
- **`/quo-plan-from-specs`** — express path when you already have a finalized PRD and SDD on disk
...
Run the bundled detector script with the target repo's absolute path. The script lives at `<this skill's base directory>/scripts/detect_fast_path.py` (where "this skill" is `quo-setup` — see *Resolve bundled helper script paths* below for the runtime-resolution convention). Replace `<bees-setup-base-dir>` with the literal path from the skill invocation header:
...
```
Base directory for this skill: /Users/.../.claude/skills/quo-setup
```

`quo-setup`'s own bundled script (`detect_fast_path.py`) lives at `<this skill's base directory>/scripts/<name>.py`. No CLAUDE.md section is written for any helper — sibling skills resolve their own bundled scripts the same way (e.g., `quo-execute` and `quo-fix-issue` compute `<quo-breakdown-epic base>/scripts/scoped_marker_resolver.py` from their own base directory — a one-hop sibling resolution into another skill's `scripts/` subtree).

**Migration.** Earlier versions of `quo-setup` wrote a `## Skill Paths` section to CLAUDE.md containing absolute machine-local paths. That section was removed because committing per-machine paths to a tracked file broke multi-engineer collaboration. If the target repo's CLAUDE.md still has a `## Skill Paths` section from an earlier setup run, delete it as part of this run — the section is no longer used by any skill, and leaving it behind keeps the broken paths in git history.
...
The canonical hive set for quorum is three top-level hives: **Issues hive**, **Plans hive**, and **Specs hive**. All three are required for the downstream skills (`/quo-plan`, `/quo-plan-from-specs`, `/quo-execute`, `/quo-fix-issue`, `/quo-file-issue`) to function — Specs is not an optional add-on.
...
- **Engineering best practices** (`docs/engineering-best-practices.md`) — coding standards, error handling conventions, module/package boundary rules, async/concurrency patterns, type design, API conventions, observability, storage patterns, security, code style, and dependency management. Ground every recommendation in the project's actual stack (e.g., for a Rust/tonic/tokio project: thiserror for errors, tracing for observability, dashmap for concurrency, clippy/fmt for style). Always include a comments section whose goal is that comments reduce the effort needed to understand and safely change the code, so a reader — including an agent — does not have to reconstruct intent or relationships from scattered implementations, tests, or design documents; keep comments accurate and specific. State it in the project's own terms and cover these points:
  - Enforce checkable facts in code (types, enums, named constants, exhaustive matches, assertions, tests) rather than in comments; keep the comment that explains why a constraint exists, what depends on it, or why a reasonable alternative would be wrong.
  - Explain intent, reasoning, and relationships — non-obvious ordering, concurrency assumptions, external constraints, tradeoffs, rejected alternatives, connections across operations or files; keep a flow overview only when it explains a dependency, invariant, or purpose the individual operations do not make clear, never a replay of them.
  - Document what a test establishes: its purpose, what its setup and assertions establish, and coverage limits; claim detection of a specific regression only when a buggy version or a deliberate perturbation made the test fail for the expected reason, and scope the claim to that evidence.
  - Keep comments that add understanding and delete the rest: keep one that explains intent, a constraint, or a relationship that would otherwise take non-obvious reasoning or investigation; delete prose that repeats a nearby name, value, operation, or assertion, plus narration, filler, and ticket bookkeeping; prefer a reference to the defining symbol over copying a changeable fact.
  - Verify quantified claims — counts, exhaustive enumerations, ordering claims, measured values — against evidence for their full scope; prefer a type, constant, assertion, or test that would fail if the claim became false; narrow unsupported claims without weakening required contracts.
  - Keep explanations close to the code, stating the rule and the local reason in the comment itself; where the project maintains a rule register, an optional citation by rule name (never a section number) may point at the broader rationale and supplements rather than replaces it.
  - Maintain comments with the code: when behavior changes, re-read every comment in every touched file and fix each one that described the old behavior — a comment true when written and false now is a defect, not a nit; verify behavioral claims against the implementation and its tests before leaving them standing; a behavior change can invalidate comments outside the files you touched, so this sweep supplements rather than replaces the project's cross-file checks; preserve useful intent before deleting.
  - Treat public documentation as a contract (protocol-schema and interface-definition comments, public-API doc comments, user- and operator-facing docs): complete enough for a consumer to use the interface without reading the implementation, exempt from the deletion heuristics above, supported but not replaced by types and tests, updated whenever the contract changes.
...
- **Compile/type-check**: Used by the Engineer agent in `quo-execute` and `quo-fix-issue` after each significant change to catch errors early before moving to the next subtask. May be empty for interpreted languages.
- **Format**: Used by `quo-execute` and `quo-fix-issue` to normalize formatting at commit time, so agent-induced reformatting in unrelated files is consistent.
- **Lint**: Used by the Engineer agent in `quo-execute` and `quo-fix-issue` at subtask boundary, and by the Code Reviewer during quality gates.
- **Narrow test**: Used by Engineer / Test Writer agents while iterating on a single file or package, to keep feedback loops fast.
- **Full test**: Used at the Task's authoritative final-validation subtask in `quo-execute`, and at the end of `quo-fix-issue` after the Engineer completes the fix.
...
- **Yes, the user has finalized PRD and SDD documents** → recommend:
  ...
  ```
  /quo-plan-from-specs <path-to-PRD> <path-to-SDD>
  ```
...
Both paths converge on the same Plan Bee shape (top-level Bee in the Plans hive with Epic children). The downstream chain — `/quo-breakdown-epic` → `/quo-execute` → `/quo-fix-issue` for any issues — works the same way for either entry point.
