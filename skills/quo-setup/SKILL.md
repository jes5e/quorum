...
- **Engineering best practices** (`docs/engineering-best-practices.md`) — coding standards, error handling conventions, module/package boundary rules, async/concurrency patterns, type design, API conventions, observability, storage patterns, security, code style, and dependency management. Ground every recommendation in the project's actual stack (e.g., for a Rust/tonic/tokio project: thiserror for errors, tracing for observability, dashmap for concurrency, clippy/fmt for style). Always include a comments section whose goal is that comments reduce the effort needed to understand and safely change the code, so a reader — including an agent — does not have to reconstruct intent or relationships from scattered implementations, tests, or design documents. State it in the project's own terms and cover: enforce checkable facts in code (types, enums, named constants, exhaustive matches, assertions, tests) rather than in comments, while keeping the comment that explains why a constraint exists, what depends on it, or why a reasonable alternative would be wrong; explain intent, reasoning, and relationships — non-obvious ordering, concurrency assumptions, external constraints, tradeoffs, rejected alternatives, connections across operations or files, and a concise overview of a complex flow even when its steps are visible; document what a test establishes and what makes it effective, without claiming a race, a coverage shape, or a detected regression its setup and assertions do not support; remove narration, filler, and ticket bookkeeping, but treat duplication alone as no grounds for deletion when a short local explanation saves a search; keep explanations close to the code and self-sufficient without the architecture doc, with references supplementing rather than replacing the reasoning; maintain comments with the code — verify behavioral claims against the implementation and tests, update on behavior change, and before deleting a comment preserve any useful intent or rationale in a clearer form, since being inferable from code is not by itself a reason to delete; and treat public documentation as a contract (protocol-schema and interface-definition comments, public-API doc comments, user- and operator-facing docs), describing behavior completely enough for a consumer to use the interface without reading its implementation, with types and tests supporting rather than replacing it, updated whenever the contract changes.
