...
|---|-------|-------|-------|--------|
| 1 | **Setup** | `/quo-setup` | A repo | `plans` and `issues` hives configured with ticket types and statuses |
| 2 | **Write specs** | (manual) | PRD and SDD documents on disk | `docs/prd.md` and `docs/sdd.md` (or similar) |
| 3 | **Plan** | `/quo-plan` (interactive) or `/quo-plan-from-specs` (express path with finalized PRD+SDD) | Feature scope (interactive) or PRD + SDD on disk | A Plan Bee (`status=ready`) with Epic children (`status=drafted`) |
| 4 | **Break Down** | `/quo-breakdown-epic` | A Plan Bee or Epic | Epics broken into Tasks and Subtasks (Epic `status=ready`) |
| 5 | **Execute** | `/quo-execute` | A Bee | Code written, committed, tests passing. Tickets `status=done` |
...
### 1. Hive Setup
If no hives exist: **"No hives configured. Run `/quo-setup` to get started."**

### 2. Spec Documents
Check if the repo has PRD/SDD documents (look at CLAUDE.md for paths, or check `docs/`).
If no specs: **"Write your PRD and SDD first, then run `/quo-plan-from-specs`."**
...
- `drafted` = written but children not yet created (needs `/quo-breakdown-epic` to create them)
- `ready` = fully planned with Tasks/Subtasks (ready for `/quo-execute`)
...
|------|-------|--------|------------|-------------|
| t1.xxx | ... | done | — | Done |
| t1.yyy | ... | ready | — | Ready for `/quo-execute` |
| t1.zzz | ... | drafted | — | Needs `/quo-breakdown-epic` |
...
- "All Epics are broken down. Run `/quo-execute b.xxx` to start executing."
- "2 Epics still need to be broken down. Run `/quo-breakdown-epic` to continue."
