...
Replace Agent Teams orchestration in `quo-execute`, `quo-fix-issue`, and `quo-breakdown-epic` with ephemeral background `Agent` invocations + a reconciliation-loop orchestrator. Ships seven custom subagent definitions (`subagents/<role>.md`), preserves all current user-visible features at the high-level UX, and removes the experimental Agent Teams dependency along with its setup ceremony (the env var, the `teammateMode` display backend, the iTerm2 hard-prompt workaround, the `force_clean_team.py` and `check_agent_teams.py` helpers).

See `docs/prd.md` `## Per-feature scope` `### Feature: Ephemeral-Agent Orchestration` and `docs/sdd.md` `## Per-feature design` `### Feature: Ephemeral-Agent Orchestration` (linked via `reference_materials`) for the canonical spec.
