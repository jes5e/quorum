...
id: b.48t
type: bee
title: 'quo-execute: don''t suggest closing Plan Bee when drafted Epics still exist'
parent: null
...
`quo-execute` mistakes "no more *workable* Epics" for "Bee is done." When a Plan Bee contains multiple Epics but only the first one has been broken down via `/quo-breakdown-epic`, the remaining Epics are still in `status=drafted`. After the first Epic completes, the skill proceeds to final review and asks the user to mark the Bee done — instead of telling them to break down the remaining Epics first.

This was hit in a real project: only the first Epic was broken down, `/quo-execute` ran on it, and after completion the agent suggested closing the Plan Bee. The other Epics in the plan were still drafted and unfinished.

There are two coupled defects in `skills/quo-execute/SKILL.md`:
...
1. User runs `/quo-plan` → Plan Bee created with Epics E1, E2, E3 in `status=drafted`.
2. User runs `/quo-breakdown-epic` on E1 only → E1 becomes `ready` with Tasks/Subtasks; E2 and E3 stay `drafted`.
3. User runs `/quo-execute` → completes E1.
...
- **Other Epics exist in `drafted` state** (or in `ready` blocked on a `drafted` dependency) → stop the loop and tell the user: "Epic <id> is complete, but Epics <ids> in this Bee are still drafted and need breakdown before this Bee can be closed. Run `/quo-breakdown-epic <bee-id>` (in a fresh session is reasonable to keep context clean) to break down the remaining Epics, then re-run `/quo-execute <bee-id>`." Do **not** proceed to Step 5 final review and do **not** offer to mark the Bee done.
...
- **UX (medium).** Defect A makes `/quo-execute` actively misleading at the close-out step. The user expects the skill to know whether their plan is done and instead gets a "ready to close?" prompt as if it were.
- **Workflow integrity.** Quorum's value is that ticket state is a reliable source of truth; both defects undermine that.

## Suggested fix

Both defects sit in `skills/quo-execute/SKILL.md`. The fix is one coherent change touching two adjacent steps:
...
No changes needed to `quo-breakdown-epic` or `quo-plan` — those skills behave correctly; the bug is purely in `quo-execute`'s end-of-loop classification and Bee-close precondition.
