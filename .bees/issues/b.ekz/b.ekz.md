...
id: b.ekz
type: bee
title: Agent Teams preflight in quo-execute / quo-fix-issue trips Bash expansion-obfuscation
  matcher
parent: null
...
The Agent Teams precondition check at the top of `quo-execute` and `quo-fix-issue` runs in the **parent** Claude Code session — before any team is spawned — and uses shell shapes that defeat user Bash allow rules. b.6k2 added a \"Shell-command etiquette\" bullet to each runtime worker role's Instructions to steer spawned agents away from these exact patterns, but the bullet only travels with worker prompts. The preflight, two screens up in the same `SKILL.md`, was not touched and still uses the offending shapes.

## Current behavior

Invoking `/quo-execute` or `/quo-fix-issue` on a properly-configured machine triggers a Claude Code permission prompt with the matcher reason \"Contains brace with quote character (expansion obfuscation)\", even when the user has previously allowed the skill set. The triggering command is the preflight check itself.

Locations:

- `skills/quo-execute/SKILL.md:38-72` — *Verifying the Agent Teams precondition* section, POSIX bash block (lines 39-56) and Windows PowerShell block (lines 60-72).
- `skills/quo-fix-issue/SKILL.md:28-62` — same section, same two blocks.
...
The preflight should run as a single stable invocation line per OS that the user can allow-list once and never re-prompt on. Per `CONTRIBUTING.md` design rule 2 (POSIX + Windows PowerShell) and the bundled-helper precedent established by `quo-setup/scripts/file_list_resolver.py` and `quo-execute/scripts/force_clean_team.py`, the logic belongs in a Python helper, not inline in `SKILL.md`.

## Impact

- Every fresh `/quo-execute` and `/quo-fix-issue` invocation prompts the user, even after the skill set is approved — a UX regression that erodes the value of allow-listing.
...
1. Add `skills/quo-execute/scripts/check_agent_teams.py` (and the same for `quo-fix-issue`, or one shared helper that both skills resolve at runtime — pick whichever matches the existing helper-resolution conventions in `docs/doc-writing-guide.md` `## Querying tickets` and `## The lookup-key pattern`).
2. Helper reads `~/.claude/settings.json` (or `%USERPROFILE%\\.claude\\settings.json` on Windows), looks up `.env.CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`, falls back to the `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` environment variable, exits 0 if either is `\"1\"`, exits 1 with the existing `Run /quo-setup first.` message otherwise. Behavior and error wording must be byte-identical to today's preflight.
...
5. Apply the same change to both `skills/quo-execute/SKILL.md` and `skills/quo-fix-issue/SKILL.md`.
...
- Both `quo-execute` and `quo-fix-issue` are updated identically; preflight error message and exit semantics unchanged.
