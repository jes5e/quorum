...
id: b.49g
type: bee
title: Wire /quo-spec-review into /quo-plan, /quo-write-prd, /quo-write-sdd as automatic quality gate
parent: null
...
Wire `/quo-spec-review` into the spec-authoring flow as an automatic
quality gate, completing the b.uxa Suggested fix step 3. Three wire-up
sites: `/quo-plan` (post both-writers, pre-Spec-Bee-promotion), and
`/quo-write-prd` and `/quo-write-sdd` (post-user-approval, pre-status-
...
`/quo-spec-review`, which is half a feature.

## Background and rationale

`/quo-spec-review` shipped under Issue `b.uxa` with skill body and
checklists complete (PRD: 8 categories tied to /quo-write-prd's 12
sections; SDD: 10 categories tied to /quo-write-sdd's 7 sections;
plus 5 cross-document consistency categories). The b.uxa commit
explicitly deferred suggested-fix step 3 ("post-write hook /
/quo-plan integration") to a follow-up Issue, citing a
...
fixed-shape PRD/SDD section structure produced by /quo-write-prd
and /quo-write-sdd, waiting to observe production failures before
...
Without the wire-up, `/quo-spec-review` is a standalone tool a human
...
Wire `/quo-spec-review` into three sites, each with consistent
...
### Site 1: `/quo-plan` Step 4c

After both `/quo-write-prd` and `/quo-write-sdd` have returned with
...
1. Invoke `/quo-spec-review <spec-bee-id>` (no `--doc` flag — both
...
4. On revision (user asks to address findings), invoke the relevant
   writer skill(s) — `/quo-write-prd` if the findings are PRD-side,
   `/quo-write-sdd` if SDD-side, both if cross-doc — with the
   findings included in the args payload so the writer can revise
   targeted sections, then re-invoke `/quo-spec-review` for a
   re-check. Apply the time-budget short-circuit (see below) so this
   does not loop indefinitely.

### Site 2: `/quo-write-prd` Step 6

When `/quo-write-prd <spec-bee-id>` is invoked standalone (not
inline from `/quo-plan`), after the user-approval gate in Step 6 but
*before* the `bees update-ticket --status ready` promotion:

1. Invoke `/quo-spec-review <spec-bee-id> --doc PRD` via the Skill
...
4. On revision, loop back to Step 5's body authoring with findings
   passed through, then re-invoke /quo-spec-review.

The inline-from-/quo-plan path skips this site (Site 1 covers it
...
`/bees-doc-review`: if a single `/quo-spec-review` invocation
...
- `/quo-plan` invokes `/quo-spec-review <spec-bee-id>` after Step
  4b's writer-skill invocations and before Step 4c's Spec Bee
  promotion. Verifiable by running `/quo-plan` end-to-end and
  inspecting the resulting flow.
- `/quo-write-prd <spec-bee-id>` invoked solo runs
  `/quo-spec-review <spec-bee-id> --doc PRD` after Step 6's user
  approval and before the PRD child's `drafted → ready` promotion.
  Verifiable by running `/quo-write-prd` standalone.
- `/quo-write-sdd <spec-bee-id>` invoked solo runs
  `/quo-spec-review <spec-bee-id> --doc SDD` after Step 7's user
  approval and before the SDD child's `drafted → ready` promotion.
  Verifiable by running `/quo-write-sdd` standalone.
- `/quo-write-prd` and `/quo-write-sdd` invoked inline from
  `/quo-plan` skip the per-writer review (Site 1 covers it
...
- `/quo-spec-review`'s frontmatter description and SKILL.md prose
...
- Changes to `/quo-spec-review`'s checklists themselves. The current
...
- A `## Spec review guide` doc-key in CLAUDE.md `## Documentation
  ...
  live inline in `/quo-spec-review`'s SKILL.md for now.
- Auto-fix or auto-revise behavior — the writer skills + their
  user-approval gates remain the only path that mutates PRD/SDD
  bodies. `/quo-spec-review` continues to return findings only.

## Dependencies

None. `b.uxa` (the original `/quo-spec-review` skill) is `done`,
