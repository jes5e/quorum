...
Track the open question of whether Claude Code's `SendMessage` tool will eventually become available without `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. If it does, the quo-execute / quo-fix-issue / quo-breakdown-epic skills should be enhanced to add warm-Agent dispatch as a token-cost optimization, restoring the original SDD intent that Epic 8s (b.5tm) had to diverge from.

## Current behavior

During the `/quo-breakdown-epic` of Epic t1.5tm.8s (under Plan Bee b.5tm — Ephemeral-Agent Orchestration), an architectural decision was made to use **per-Subtask cold dispatch** for Engineer/Test Writer/Doc Writer Agents in the rewritten quo-execute (and by extension quo-fix-issue and quo-breakdown-epic in Epics C/D).
...
- Epic where the divergence is implemented: t1.5tm.8s (quo-execute rewrite).
...
- 2026-08-14 — **Closed as wontfix** per maintainer decision (lifecycle branch (b): drop the optimization permanently). The warm-Agent re-probe is no longer useful; per-Subtask cold dispatch with prompt caching is the settled architecture. No follow-up Plan Bee will be filed. The in-skill divergence callout in `skills/quo-execute/SKILL.md` Section 3 and the SDD divergence note remain valid as historical record.
