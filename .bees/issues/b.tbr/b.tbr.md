...
When `/bees-execute` runs against a Plan Bee that was authored by the
*pre-redesign* `/bees-plan` (one that mutated `docs/prd.md` and
...
entry alongside the stale entry the old `/bees-plan` already wrote.
...
If either was authored by the pre-redesign `/bees-plan` Step 4 doc-
...
*implementation*). When `/bees-execute` is eventually run against
...
4. Once verified or remediated, proceed with `/bees-execute b.9xr`
   and `/bees-execute b.gar` as planned. Visually inspect the
...
- **Before** `/bees-execute b.9xr` or `/bees-execute b.gar` runs.
