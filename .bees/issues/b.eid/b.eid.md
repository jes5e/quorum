...
2. **Execute mode needs a receiver for the Engineer's `## Design question` return.** b.q3f's `agents/engineer.md` rule is mode-independent: when an assignment needs a mechanism the approved design (or Task body) did not enumerate, the Engineer implements the rest, stops, returns the question under `## Design question`, and in execute mode leaves the Subtask at `in_progress` (does not run `status=done`). `/quo-fix-issue` receives that return as a Section 3 Revise by reference (re-dispatch the Analyst; the active Analyst task is the carrier; no gate fires inside Phase A). `/quo-execute` has no receiver: its Reconcile step persists every Engineer return as a completion by confirming the `status=done` transition, so today the only tell is a Subtask that stays `in_progress`. Design the execute-mode receiver alongside the site list so both land with one vocabulary — the natural route is back to the checkpoint that authored the Task body (the breakdown PM) rather than a per-Subtask Analyst, per this Issue's own "do not add a per-Subtask Analyst dispatch" rule. The Section 8 (post-completion) analogue in `/quo-execute` needs the same look; `/quo-fix-issue` routes a post-completion design question to its File-as-issue branch.
