...
3. **Self-short-circuit in `agents/pm.md`** for the no-spec-surface case. When the PM determines there's no meaningful spec to verify against (issue body is fully self-describing, no PRD/SDD reference in the body, `up_dependencies` does not include a Plan Bee, `reference_materials` is null *or* the upstream URL content fetched via `WebFetch` is trivially short — e.g. a one-sentence rename ticket), the PM exits quickly with a one-line verdict like "no spec drift surface to review for this Issue" and skips the deep review. The orchestrator reads that as a clean signal and includes it in the per-issue summary.
...
6. **Section 4 reviewer dispatch block (lines 344-368)** — add PM to the conditional-spawn rules at lines 348-353 as always dispatched (no implementer-presence gate, matching the always-dispatched Doc Writer pattern). Update the dispatch-shape opener at line 346 to name PM as a fourth Agent dispatched alongside the three reviewers. Add a corollary that PM dispatches alongside the reviewers, not instead of any of them.
...
8. **Section 5 (lines 370-390)** — collapse the Complex/Simple branches into a single path. PM's spec-alignment finding always lands in the per-issue summary (the PM was dispatched in Section 4 and has already verified spec alignment), so Section 5 is always informational confirmation, never load-bearing orchestrator-direct work. The line-374 phrase "the per-issue PM Agent dispatched in Section 3" becomes "the per-issue PM Agent dispatched in Section 4."
...
- **Drop the Complex gate; move PM to Section 4; add a self-short-circuit (the chosen path).** Accepted: addresses all three concerns (whether, when, what) with a single coherent design. Matches the always-dispatched precedent set by Doc Writer in the same skill. Costs no extra Agent invocations on the steady state — the PM was already dispatched on Complex Issues; now it's dispatched on all Issues but short-circuits cleanly on the no-spec-surface case. Section 4 timing means the PM always has a diff to review when the short-circuit path does not fire.
