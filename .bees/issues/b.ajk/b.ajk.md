...
- Nothing tells the user what session setting a given skill is tuned for. The current setting is readable from `CLAUDE_EFFORT` (see Verified below), but no skill reads it.
...
2. **The session's current effort is readable, live, and exported on every launch path.** `printenv CLAUDE_EFFORT` returns the resolved session effort. Verified across three conditions:
   - Launched with an explicit `--effort xhigh` flag, it returns `xhigh`.
   - Changed to `high` mid-session via `/model` and re-read, it returns `high` — so the value tracks the current setting rather than echoing the launch flag.
   - Restarted as a new process (confirmed by a changed `CLAUDE_PID`) with **no** `--effort` flag and a saved default of `high`, it returns `high` — so the variable is populated from resolved session state, not conditional on an explicit CLI argument.

   Change 3's gate can therefore read the operator's actual current setting regardless of how the session was started. The unset-fallback specified in change 3 remains as defence for older CLI versions but is not expected to be the common case. Re-run this three-condition check if the gate ever appears to misfire.
...
- **Deferred: escalate effort on review failure.** Re-dispatching the Engineer at `xhigh` after a Code Reviewer returns findings twice is sound in principle, but it is unproven, adds per-Subtask retry bookkeeping to the orchestrator, and depends on a capability this ticket did NOT verify: whether an effort override can be passed per Agent dispatch, as opposed to pinned in frontmatter. Verify that separately before picking this up. Not in this ticket.
