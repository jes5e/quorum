...
- `agents/<role>.md` — the eight custom-subagent role contracts (`engineer`, `test-writer`, `doc-writer`, `pm`, `code-reviewer`, `test-reviewer`, `doc-reviewer`, `analyst`), each with frontmatter (`name`, `description`, `model`, `effort`, `tools`) and the role's instructions. `quo-fix-issue` dispatches the Analyst on every Issue outside its `text` class, `quo-execute` only on escalation; `quo-breakdown-epic` dispatches the PM for its traceability review; the two execution skills dispatch the implementer and reviewer roles.
...
2. **State that must survive what the agent cannot observe** — a compaction, or the next session's blindness to this conversation. Keep the minimum the rest can be re-derived from, plus one recovery goal: re-read it and reconcile it with the tickets and the working tree before acting.
...
- A rule of kind 5 needs a failure observed in a real run and recorded in a ticket, run report, or commit. A scenario a reviewer role-plays or predicts is a prediction, and earns one goal sentence at most. This evidence bar does not apply to kinds 1–4, or to the design of a new feature's contracts.
...
**A tool call fronts every gate.** Every `AskUserQuestion` fired as a workflow gate is preceded, in the same turn, by a tool call: a run-state-manifest write in `/quo-execute`, `/quo-fix-issue`, `/quo-plan`, and `/quo-breakdown-epic`, and the write of the body the gate asks about in solo `/quo-write-prd` / `/quo-write-sdd`. That tool call is a structural guard against the narrate-instead-of-do failure that prose-only counter-anchors did not close. The two-step `TaskCreate` form it replaced survives only in the three code/test/doc review skills' clause for other callers (`docs/doc-writing-guide.md` `## The two-step TaskCreate → prescribed-tool contract`); `CONTRIBUTING.md` `## Known limitations` documents the residual failure surface.
