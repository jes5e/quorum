...
- **Cold-review exit rule for hand-edit batches.** A finding that changes behavior (a dispatch condition, routing input, relay recipient, staging, commit, or gate) earns a fresh cold round regardless of its severity tag. A round that returns only wording findings on sentences two or more earlier cold passes accepted unchanged closes the lane: trivial fixes are applied and the findings are recorded in the commit message as accepted residuals. This is the deferral disposition the nit rule already sanctions, not a round cap, and it reviews nothing less — it stops paying a full round for reviewer-to-reviewer wording variance. Batches stay small: a hand-edit batch that grows past one validation report's worth of items is split and committed in pieces.
