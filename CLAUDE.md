...
**Two-step gate-task contract.** In `/quo-breakdown-epic`, and any skill that does not front its gates another way, every `AskUserQuestion` fired as a workflow gate — whether a dispatched skill's routing trailer or orchestrator prose prescribes it — first creates a `gate-askuserquestion-<short-suffix>` TaskList task naming the gate, then calls `AskUserQuestion` in the same turn. That tool call in the gate's turn is a structural guard against the narrate-instead-of-do failure that prose-only counter-anchors did not close. `/quo-execute`, `/quo-fix-issue`, and `/quo-plan` make it a run-state-manifest write instead, and solo `/quo-write-prd` / `/quo-write-sdd` the write of the body the gate asks about. The load-bearing specification, including the yield-control discipline that mirrors the `defer-*` rule, is `docs/doc-writing-guide.md` `## The two-step TaskCreate → prescribed-tool contract`; `CONTRIBUTING.md` `## Known limitations` documents the residual failure surface it narrows but does not close.
