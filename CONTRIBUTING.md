...
- **Frontmatter:** Only `name` and `description` are honored by Claude Code's skill loader. `triggers:` and `disable-model-invocation:` are silently ignored. Don't add them to new skills; remove them from existing skills if you spot them. This applies to `skills/<name>/SKILL.md` only — the custom-subagent loader that reads `agents/<role>.md` honors a larger set (`name`, `description`, `model`, `effort`, `tools`), which is how per-role model and reasoning-effort assignments are pinned. **This bullet is the canonical enumeration of the agent-file key set; elsewhere in this file, point here rather than re-listing the keys.**
...
- **Don't add frontmatter keys Claude Code ignores.** Keeps the source honest about what's actually consumed. Note the two loaders honor different key sets: a `skills/<name>/SKILL.md` gets only `name` and `description`, while the agent-file loader honors more (see the frontmatter bullet under *Skill conventions* for the canonical list). This anti-pattern forbids inert keys, not the agent-file keys.
...
- **Adding `triggers:` and `disable-model-invocation:` frontmatter to skills** — rejected: Claude Code silently ignores any `skills/<name>/SKILL.md` frontmatter beyond `name` and `description`. The keys don't actually wire up trigger phrases or anything else. **This applies to skill files only.** The custom-subagent loader that reads `agents/<role>.md` honors a larger key set, all of which is consumed (see the frontmatter bullet under *Skill conventions* for the canonical list). Pinning a role's model or reasoning effort in agent frontmatter is therefore in-bounds and is how the per-role assignments in CLAUDE.md `## Model assignment in execution skills` are expressed; don't read this rejection as forbidding it.
