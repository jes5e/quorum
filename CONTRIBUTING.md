...
- **`/quo-breakdown-epic` dispatches only read-only agents.** Its `Explore` researchers and its `pm` reviewer change nothing; only the orchestrating skill drafts and creates tickets. The execution skills (`/quo-execute`, `/quo-fix-issue`) let dispatched implementers change files and create commits, not tickets — different scope of authority.
