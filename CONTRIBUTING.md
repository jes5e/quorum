...
- **The Engineer's design-question rule has a receiver in fix mode only.** `agents/engineer.md` requires an Engineer to stop rather than invent a mechanism the approved design or the ticket body did not enumerate, and to return the open question under `## Design question`. The rule is deliberately mode-independent, but only `/quo-fix-issue` has rungs that receive such a return, and it has two, routed differently because their surroundings differ: the design-question rung routes a forward-fix-path return back to the Analyst as a Revise, while the post-completion pass — which has no Analyst to re-dispatch, and runs after the Issue is `done` and committed — routes the return to its **File as issue tickets** branch as a filed follow-up Issue carrying the question. `/quo-execute` has no receiver and no upstream design analysis to revise, so an execute-mode design question surfaces as a partial diff on a Subtask left at `in_progress` — detectable, because the status ladder is the handoff signal, but routed by the orchestrator's own judgement rather than by a rule. Do not read the asymmetry as an oversight in `agents/engineer.md`: the rule binds in both modes on purpose, because inventing an unenumerated mechanism is the failure it prevents either way, and the mode-independence is what keeps the returned question available to whatever receiver execute mode eventually grows. Designing that receiver — and the upstream relay that would give execute mode a site list to revise against — is recorded in Issue `b.eid`, under the `## Deferred from /quo-fix-issue run` section appended to that Issue's body when the fix-mode half shipped, rather than in the Issue's original expected-behavior list.
