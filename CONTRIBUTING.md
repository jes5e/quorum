...
4. **Body-as-spec is a supported mode.** When a Plan Bee's `reference_materials` is null (no skill emits that shape today; older and hand-made Plan Bees carry it), the Plan Bee body itself is the authoritative spec. Downstream skills (`/quo-breakdown-epic`, `/quo-execute`, `/quo-fix-issue`) all have explicit prose for this — don't simplify it away on a future refactor.
5. **Cumulative docs preferred but not required.** When a feature ships, the Doc Writer adds a new `### Feature: <title>` subsection under cumulative `## Per-feature scope` / `## Per-feature design` headers; old features stay documented. But the workflow does not enforce this — projects with monolithic specs work fine too.
6. **Goals, not procedures.** Skill prose states goals with their reasons and gives exact text only where it has to. CLAUDE.md `## How skill prose is written` defines where that is, and how a skill improves over time.
...
- **Structured file edits:** When a skill edits a structured file (JSON, a markdown section), state the goal — parse the file, change the named key or section, write it back atomically — and why: a regex cannot tell a `## ` heading inside a fenced code block from a real section boundary, so it silently eats too little or too much, and a direct text edit has no atomicity and corrupts the file on a wrong escape. A bundled helper script carries the code only when the edit is non-trivial and used in two or more places (see *Anti-patterns*). On Windows, Python is invoked as `python` (or `py -3`); `python3` is generally not on PATH for python.org installs.
- **Run-state, not TaskList:** no skill reads TaskList state; the tools are off by default. Run-scoped state that routes work (lanes, obligations, open gates) lives in the skill's run-state manifest. A new obligation kind needs its close-out site authored in the same change as its creation site — including the skill's abort path — since an obligation left open past a unit boundary silently blocks a later phase.
...
- **`/quo-breakdown-epic` dispatches only read-only agents.** Its `Explore` researchers (which hold no `Edit` or `Write`) and its `pm` reviewer change nothing; only the orchestrating skill drafts and creates tickets. The execution skills (`/quo-execute`, `/quo-fix-issue`) let dispatched implementers change files and create commits, not tickets — different scope of authority.

## Anti-patterns

- **Don't proliferate bundled scripts.** Each new helper in `scripts/` is install-mode coupling and maintenance burden. State one-off operations (JSON edits, simple file ops) as goals in skill prose. Add a script only when the operation is non-trivial AND used in 2+ places.
...
- **Don't write vague prose** ("run the appropriate test command"). Prose that leaves the agent to guess gets guesses, and they are often wrong. Name exactly where the answer is instead: a contract key (the `Full test` command under CLAUDE.md `## Build Commands`), or, for an external CLI, the tool and the facts its help does not make obvious. That is different from spelling out an external CLI's invocations, which go stale when the tool changes; see principle 6.
...
- **Dedicated bundled helper script for JSON edits to `~/.bees/config.json` and Claude Code `settings.json`** — rejected: skill prose stating the edit as a goal (parse, change, write atomically) gets the same atomicity without adding a maintained file.
...
- **The unexplained-movement gate has no retry-first step.** When a writer reports that the source moved underneath it and the orchestrator cannot attribute the movement — to the Engineer, to a concurrently-dispatched Test Writer's recorded perturbation, or to an external actor — the movement rung in each execution skill's gates section fires a three-option gate (**Re-dispatch the writer now** / **Wait** / **Abort this Issue** in `/quo-fix-issue`, **Abort this unit** in `/quo-execute` — execute mode has no Issue concept, and its abort always ends the run). That gate is the deliberate design for the unattributable case and fires **only** there; attributable movement re-dispatches or waits with no gate. A retry-once refinement ahead of it — re-dispatch the writer a single time and gate only on a second abort — was not evaluated when the gate was designed; it is a plausible future simplification, not a defect in what shipped.

- **The Engineer's design-question rule has two receivers per skill, routed differently.** `agents/engineer.md` requires an Engineer to stop rather than invent a mechanism the approved design or the ticket body did not enumerate, and to return the open question under `## Design question`. On the forward path both orchestrators route such a return to the Analyst as a Revise — `/quo-fix-issue` against the proposal it approved, `/quo-execute` (since 2026-09-18, Batch B2a of `b.vtw`) against the tickets from the escalation's scope up to the Bee and the spec, with an approved revision appended to the ticket at the escalation's scope — while the post-completion pass in either skill, which runs after the unit is committed so no revision can land, routes the return to its **File as issue tickets** branch as a filed follow-up Issue carrying the question. The residual limitation: an execute revision has no durable carrier until its Approve, so a compaction in that window re-derives it through the Analyst, and a `recommend-different-approach` revision can leave the Task body stale (the revision names what it supersedes, and a PM finding on the contradiction takes the premise check).
...
- **Does NOT apply to:** non-user-facing state-mutation tool calls. Concretely, a status promotion such as `bees update-ticket --status ready` runs *without* a paired `gate-*` task because no user prompt fires; the same is true for other workflow-internal state mutations the orchestrator may emit.
...
**Why the skills carry two post-compaction rules, not one.** That asymmetry is why each orchestrator skill pairs an **unconditional** rule — conversation memory is never a substitute for the durable sources (bees, git, and the run-state manifest); re-read them on every tick — with a **conditional** one that fires only when a summarization marker is visible (treat everything before it as non-authoritative and re-read all sources in full before dispatching). The unconditional rule is **not** redundant with the conditional one and must not be pruned as such: the conditional rule covers only the case the harness announces, and microcompaction announces nothing.
...
**Design consequence.** The three orchestrator skills use **state-externalization checkpoints plus harness-owned compaction** instead of any self-clear. `/quo-execute` carries the **Epic-boundary state-externalization checkpoint** and `/quo-fix-issue` the **Issue-boundary state-externalization checkpoint**; `/quo-breakdown-epic` rewrites its manifest at each Epic boundary. At each boundary the orchestrator verifies that all load-bearing state is already in a durable carrier (bees ticket statuses, per-unit commits, the compromise tracker in the two execution skills — `/quo-breakdown-epic` has none — and the `defer-*` ledger in the manifest's `## Obligations`, closed at the Issue boundary in `/quo-fix-issue` and once at run end in `/quo-execute` and `/quo-breakdown-epic`), rewrites the run-state manifest under `<tempdir>/.quorum/`, and treats the prior unit's conversation as non-authoritative from that point on. Context is not reclaimed by any of this — it grows monotonically until the harness compacts — but the compaction is **lossless whenever it fires, including mid-unit**, and a post-compaction reconciliation tick can re-derive the whole run from the carriers. The reclamation lever that does exist is the user starting a **fresh session** at a boundary. The general rule this case produced is the **executable-verb test** in CLAUDE.md `## Review criteria for skill changes`.
