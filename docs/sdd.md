...
**Recursive delegation: not supported.** The data model (Bee → Epic → Task → Subtask) is naturally hierarchical and the architecture would in principle support an Epic-level sub-orchestrator Agent that internally manages its Tasks via further Agent invocations. The probe completed during Epic 8s (Probe results recorded under `t2.5tm.kn.vt`) resolved the open question: per the [Claude Code sub-agents docs](https://docs.claude.com/en/docs/claude-code/sub-agents), "Subagents cannot spawn other subagents" — only the top-level orchestrator may dispatch Agents. All three execution skills (`bees-execute`, `bees-fix-issue`, `bees-breakdown-epic`) therefore ship **flat orchestration**: every Agent invocation originates from the skill's reconciliation loop, never from a worker. The bound on flat-orchestration context growth is the **Epic-boundary context-clear discipline** (defined in `bees-execute/SKILL.md` Section 4.2), which clears the orchestrator's working context between Epics and keeps the loop's running set bounded at roughly ~25-30% of the 1M context window per Epic.
...
**Updated SDD sections (landed during Epic fy):**
