...
How to write and review tests for this repo's own artifacts — the bundled Python helper scripts under `skills/*/scripts/`. There is no application package; the suite exists to pin the helpers' CLI contracts. The existing modules under `tests/` are the idiom of record: when this guide and a well-established pattern in those modules disagree, match the modules and file an issue against this guide.
