...
**Trailer-less gates also honor the contract.** Some orchestrator-driven gates fire `AskUserQuestion` without a dispatched-skill trailer prescribing it — e.g., `/quo-fix-issue`'s Analyst gate; `/quo-plan`'s plan-approval gate; the unexplained-movement gate in each execution skill's gates section, fired when a writer stops on source movement the orchestrator cannot attribute. These gates ALSO go through the two-step `TaskCreate` → `AskUserQuestion` contract (in `/quo-execute`, `/quo-fix-issue`, and `/quo-plan`, the manifest `## Open gate` write takes the `TaskCreate` step's place): the orchestrator prose itself is the source of the prescription rather than a returned trailer, but the structural improvement (a `TaskCreate` tool call is hard to silently yield) applies identically. When you author or edit such a gate, surface the two-step obligation in the orchestrator prose at the gate-firing site with the same `gate-<kind>-<short-suffix>` naming convention.
