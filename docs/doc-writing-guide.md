...
**Yield-control discipline (orchestrator side).** No skill yields control to the harness with active `gate-*` TaskList tasks remaining in `pending` or `in_progress` status. Each consuming skill's trailer-consumption site (`/quo-plan` Step 4c and Step 5e, `/quo-execute` Section 5 and Section 3, `/quo-fix-issue` Section 5, Section 4, and Section 3, `/quo-breakdown-epic` Section 5) MUST honor this discipline at the same place its `defer-*` discipline lives — the `gate-*` lifecycle is shorter (created and completed within a single turn under normal operation) but the active-set rule is the same shape. The consumption-site prose should reference this section rather than re-deriving the rationale.

**Trailer-less gates also honor the contract.** Some orchestrator-driven gates fire `AskUserQuestion` without a dispatched-skill trailer prescribing it — e.g., `/quo-fix-issue` Section 3's user-approval gate after the Analyst returns; `/quo-plan` Step 5e's plan-reviewer-Agent verdict gate; the unexplained-movement gate in each execution skill's reconciliation loop (`/quo-fix-issue` Section 4, `/quo-execute` Section 3), fired when a writer stops on source movement the orchestrator cannot attribute. These gates ALSO go through the two-step `TaskCreate` → `AskUserQuestion` contract: the orchestrator prose itself is the source of the prescription rather than a returned trailer, but the structural improvement (a `TaskCreate` tool call is hard to silently yield) applies identically. When you author or edit such a gate, surface the two-step obligation in the orchestrator prose at the gate-firing site with the same `gate-<kind>-<short-suffix>` naming convention.
...
The two execution skills carry a third non-Agent name class on the same substrate — `aborted-<role>-<scope>`, the redelivery-owed marker opened when a Test Writer or Doc Writer stops mid-run on detected source movement. Its lifecycle sits between the other two (unit scope: opened when the writer aborts, closed when the re-dispatched lane delivers), and like `defer-*` and `gate-*` it is an **obligation expressed as a pending task** rather than a record of an Agent. When you add a name class of this kind, give it a distinct prefix that no existing routing test matches by accident, and author its close-out site in the same change as its creation site — including on the skill's abort path, since a marker stranded `pending` past a unit boundary silently blocks a later phase.
