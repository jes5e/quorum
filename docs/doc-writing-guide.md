...
- **Orchestrator-side prose stays as reference context, with a pre-commitment line at each call site.** When you edit an orchestrator site that consumes a review skill's output (`/quo-plan` Step 4c, `/quo-write-prd` Step 6a, `/quo-write-sdd` Step 7a, and the review loops of `/quo-execute` and `/quo-fix-issue`), keep the trailer-is-authoritative callout intact — it tells the orchestrator agent to follow the trailer literally and that any divergence between trailer and prose is a bug in the review skill to file. Additionally, keep the **pre-commitment line** immediately before each `/quo-spec-review` Skill invocation — a sentence in the shape `When the Skill call returns, you MUST FIRST create a gate-<kind>-<short-suffix> TaskList task (per ## The two-step TaskCreate → prescribed-tool contract in docs/doc-writing-guide.md), THEN call the prescribed tool (AskUserQuestion when findings are present, bees update-ticket --status ready when no findings). The dispatched skill's trailer will repeat this two-step obligation; treat the trailer as a confirmation, not a new instruction. A text-only response between the Skill return and that tool use is a defect.` This pre-commitment is the complementary half of the second-person-imperative trailer: it relocates attentional weight from end-of-response (where it is easy to drop) to immediately-before-Skill-call (where it is easy to recall when the Skill return lands).
...
**Scope — user-facing gate prescriptions only.** The contract triggers when the prescribed tool call is a **user-facing gate** — `AskUserQuestion` today, or any future tool whose absence stalls the workflow on a user-visible prompt. The contract does **not** apply when the prescribed tool call is a non-user-facing state mutation like `bees update-ticket --status ready` or `bees create-ticket` — those calls run without a user prompt, and the failure mode the contract exists to mitigate (silently yielding a user-facing prompt the user has no way to surface manually) does not arise for them. Concretely: `/quo-spec-review`'s no-findings Shape-3 trailer prescribes `bees update-ticket --status ready` and runs *without* a paired `gate-*` task because there is no user choice to silently yield; its findings-present Shapes 1 and 2 prescribe `AskUserQuestion` and DO carry the two-step contract because the prompt is user-facing. The b.wii Issue body and the Analyst proposal both framed the failure mode specifically around silent `AskUserQuestion` yields, so scoping the contract to user-facing gates matches the original framing and avoids `TaskCreate` overhead at every workflow-internal tool call. If a future Issue extends the structural mitigation to non-user-facing tool calls, that is a separate scoping decision that should re-evaluate the cost-vs-coverage tradeoff explicitly rather than expanding silently here; `CONTRIBUTING.md`'s known-limitations section records this scoping decision alongside the residual-failure-surface discussion.

**Yield-control discipline (orchestrator side).** No skill yields control to the harness with active `gate-*` TaskList tasks remaining in `pending` or `in_progress` status. Each consuming skill's trailer-consumption site (`/quo-plan` Step 4c and Step 5e, `/quo-breakdown-epic` Section 5; `/quo-execute` and `/quo-fix-issue` front their gates with a run-state-manifest write from the manifest write onward, fire the three run-start gates that precede it directly, and create no `gate-*` tasks) MUST honor this discipline at the same place its `defer-*` discipline lives — the `gate-*` lifecycle is shorter (created and completed within a single turn under normal operation) but the active-set rule is the same shape. The consumption-site prose should reference this section rather than re-deriving the rationale.

**Trailer-less gates also honor the contract.** Some orchestrator-driven gates fire `AskUserQuestion` without a dispatched-skill trailer prescribing it — e.g., `/quo-fix-issue`'s Analyst gate; `/quo-plan` Step 5e's plan-reviewer-Agent verdict gate; the unexplained-movement gate in each execution skill's gates section, fired when a writer stops on source movement the orchestrator cannot attribute. These gates ALSO go through the two-step `TaskCreate` → `AskUserQuestion` contract (in `/quo-execute` and `/quo-fix-issue`, the manifest `## Open gate` write takes the `TaskCreate` step's place): the orchestrator prose itself is the source of the prescription rather than a returned trailer, but the structural improvement (a `TaskCreate` tool call is hard to silently yield) applies identically. When you author or edit such a gate, surface the two-step obligation in the orchestrator prose at the gate-firing site with the same `gate-<kind>-<short-suffix>` naming convention.
