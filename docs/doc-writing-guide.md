...
**Yield-control discipline (orchestrator side).** No skill yields control to the harness with active `gate-*` TaskList tasks remaining in `pending` or `in_progress` status. No skill in this set creates `gate-*` tasks today: `/quo-execute`, `/quo-fix-issue`, `/quo-plan`, and `/quo-breakdown-epic` front their gates with a run-state-manifest write (the run-start gates that precede it fire directly), and the solo spec writers front their gate with the write of the body under review. A caller that does use the two-step contract honors this discipline where its `defer-*` discipline lives: the `gate-*` lifecycle is shorter (created and completed within one turn) but the active-set rule is the same shape.
