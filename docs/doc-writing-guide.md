...
The four review skills (`/quo-spec-review`, `/quo-engineer-review`, `/quo-test-writer-review`, `/quo-doc-writer-review`) emit a `**Next action for the orchestrator:**` trailer line at the bottom of their output, naming the precise gate or follow-up the consuming orchestrator must run after reading the findings. The trailer is the **authoritative routing prescription** for the orchestrator — the surrounding orchestrator-skill prose is reference context, not a load-bearing rule the orchestrator must recall from memory. When you edit a review skill, keep the trailer emit in lockstep with the work-item shapes the skill returns:

- **Emit verbatim, do not paraphrase.** The trailer phrasings are load-bearing; per-invocation drift defeats the move-to-artifact purpose that motivated the trailer convention. The literal `**Next action for the orchestrator:**` prefix, the literal `AskUserQuestion`-with-finite-choices language, and the literal "Do not yield without doing this" enforcement clause (where applicable) must survive verbatim through any prose edit.
- **One trailer shape per output shape.** `/quo-spec-review` collapses to three shapes keyed on severity (blockers present / suggestions-or-nits only / no findings); the three code/test/doc reviewers collapse to two (findings present / no findings). When you add a new output shape, add the matching trailer shape in the same edit.
- **Standalone path emits too.** Even when the review skill is invoked standalone by a user, the trailer still emits — the "orchestrator" it addresses is the assistant turn presenting the findings to the user. Don't suppress the trailer on the standalone path; its presence is a self-check that the routing question was considered.
- **Orchestrator-side prose stays as reference context.** When you edit an orchestrator site that consumes a review skill's output (`/quo-plan` Step 4c, `/quo-write-prd` Step 6a, `/quo-write-sdd` Step 7a, `/quo-execute` Section 5, `/quo-fix-issue` Section 4), keep the trailer-is-authoritative callout intact — it tells the orchestrator agent to follow the trailer literally and that any divergence between trailer and prose is a bug in the review skill to file.
