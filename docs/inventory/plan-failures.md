...
- **S3a — Doc children stay `drafted` until approval.** An inline writer that updates an existing child sets it back to `drafted`. When `/quo-plan` reuses a `ready` Spec Bee, it sets the Spec Bee to `drafted` for the run. On approval `/quo-plan` promotes the children, then the Spec Bee (SV12). Without this, removing the inline writer gates would let unapproved bodies sit in `ready` tickets after a Cancel (cold review, finding 2).
...
| P32 | L168 | Normalized title match; err toward reuse; ask when ambiguous; maintainers must not tighten | O + A | O5 | Goal → SV6; counter-anchor goes |
| P33 | L170-174 | Reuse gate: `Reuse existing Spec Bee` / `Create a new Spec Bee anyway` / `Cancel` | C | O5 | Keep → SV6 (fires only on a candidate) |
...
- **SV6 Spec Bee** (P29, P31–P34): reuse by normalized title, with a gate only on a candidate (O5); otherwise create `drafted` with a short body. Specs live in tickets so planning never touches project docs (F2).
...
- **Item 1, as an SDD contract** (SV21). `## Requirements` gains two required subsections:
  ...
  Each has an explicit `none` line when empty. The mechanism vocabulary is not restated. It points at the definition in `agents/analyst.md`, as b.eid's own note asks; the file ships with the agents, so the pointer holds downstream. A named heading gives b.pcc something to cite when it builds per-Task site lists (cold review, finding 8). The SDD keeps its seven top-level sections, so spec review's section count and the existing readers don't change.
