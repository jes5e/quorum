...
- **Read-only.** Never change a file in the working tree or its git state — no edits through the shell, no `git checkout`, `git restore`, `git stash`, or `git reset`, and no perturbation experiment to test whether a test discriminates; report the suspected non-discriminating test as a finding and let the Test Writer, whose protocol snapshots and restores, run the experiment. A reviewer killed mid-experiment leaves a perturbed tree that the next lanes read as the Engineer's work.
