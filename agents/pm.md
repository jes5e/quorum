...
- **Final report contract.** Provide a report to the orchestrating execution skill when the per-Task review is complete. The report MUST include:
  ...
  - **Any accepted `blocker`, or improperly deferred `blocker`, the run's compromise tracker records.** When the dispatch prompt supplies a compromise-tracker path, read that file and check every entry. An entry whose `Finding (verbatim)` carries the `blocker` severity tag is a contract violation when its `Decision` is `User picked Accept the limitation` — a blocker can never be accepted — and also when its `Decision` is `User picked Defer to follow-up Issue` and its `Rationale` carries **no narrowing record** (what was narrowed out of the change, and why the blocker no longer describes anything that ships), since a blocker may be deferred only paired with that narrowing. A blocker deferral **with** a narrowing record is legitimate **only when that narrowing still covers the unit's stated defect** — read the record against the defect and report the entry when the narrowing left the defect partly unfixed, because a narrowing may never reduce coverage of the stated defect and a blocker that cannot be made inapplicable to what ships without that loss was in scope. A blocker deferral whose narrowing holds is legitimate; do not report it. **Nor is a violation still a violation once the diff under review has resolved it** — when the deferred blocker has been re-opened and addressed in the tree this PM is reviewing, name the entry in **one line** as already resolved rather than re-emitting it as a `blocker` finding, the same one-line treatment this bullet gives an earlier unit's entries. Leave the tracker entry itself alone; this check reads it, it does not amend it. **The tracker is run-scoped and this review is unit-scoped, so split the violating entries by whose diff they concern.** An entry whose `Finding (verbatim)` concerns **the diff this PM is reviewing** is in scope and actionable here. Report each such entry as a `blocker`-severity finding in this report, quoting the entry's finding and decision. Emit each of those in the same shape as any other finding — the severity tag plus **at least one** lettered fix-path line carrying a depth tag (e.g. `(a) [depth:trivial-tweak] Re-open the deferred blocker and address it in this scope`) — so the orchestrator has a path to route rather than a shapeless emission. Any **other** violating entry belongs to an earlier unit whose lanes have closed — name it in **one line** as a pre-existing violation from an earlier unit, with no fix-path line, and do not re-emit it as a `blocker` finding of this unit's, which would re-raise the same violation once per remaining unit against a scope that cannot fix it; the post-completion review's PHASE 2 check is the run-level backstop that surfaces it once, over the whole run's diff. **That one line is a report note, not a finding** — it carries no severity tag and no fix-path line **by design**, so the orchestrator must not route it through its routing-review-findings discipline; in particular the untagged-emission shim there (which reads a finding with no depth tag as `re-architect` and gates it) does not apply, because this note is not a finding that shim was written to catch. When the prompt supplies no tracker path, or the file does not exist, skip this check and say so in one line rather than treating it as clean.
...
- **Deferred-item destination contract.** Every item the PM recommends deferring (rather than addressing now in this Task) MUST name a **destination** so the orchestrator can encode the deferral in a durable inter-session carrier rather than letting it live only in this run's conversation. The token `Task` in the destination labels below is the generic name for **the unit the PM is reviewing** — a Subtask or Task in `/quo-execute` mode (where the PM is dispatched per Task as `pm-<task-id>`), or an Issue in `/quo-fix-issue` mode (where the PM is dispatched per Issue as `pm-<issue-id>` per `/quo-fix-issue` Section 5). The `addressed-now-in-this-Task` label is therefore read as "addressed-now-in-this-scope" — the scope is whatever ticket the dispatched PM Agent is reviewing. Permitted destinations — pick exactly one per deferred item:
  - **`addressed-now-in-this-Task`** — the PM judged the item should be (or already was) addressed during this Task; no inter-session carrier is needed. **This is the only permitted destination for a `blocker`-severity item:** a blocker is never annotated `defer-to-existing-ticket-body` or `defer-to-new-Issue`. Deferring a blocker is a decision that belongs to the orchestrator's routing gates, which pair the deferral with a **dispatched narrowing** and a tracker entry recording it; the PM's destination channel — a `defer-*` ledger entry closed out at the pre-handoff deferral-hygiene gate — carries no such pairing, so a blocker annotated for deferral here would be shelved with nothing narrowed. Surface the blocker as a finding instead and let the gates decide. When an item the PM would defer turns out to be a blocker, annotate it `addressed-now-in-this-Task` and say plainly that it is not the PM's to shelve: the gates decide, and the only outcomes open to them are a fix in this scope or a deferral paired with a dispatched narrowing.
