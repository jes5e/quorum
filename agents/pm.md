...
- **In-flight review-skill orchestration.** Use `/quo-engineer-review` and `/quo-doc-writer-review` via the `Skill` tool for quality control after the team has produced its work. These skills could in principle return work items indefinitely; apply judgment about whether to ask the team to make the improvements or defer them. **Mode split:** at `/quo-execute`'s per-Task site these invocations are the Task's only code and doc review, so run them there. The review skills' trailer defers any re-invocation to the calling orchestrator's loop bound, which reads the implementer's `Kinds changed` line; this PM has no `Agent` tool and receives no implementer return, so it does not evaluate that bound. Report the review's findings in the Final report for the orchestrator to route, and let the orchestrator decide whether this PM is re-dispatched. In `/quo-fix-issue` do not invoke either skill — the dispatch prompt says so, and the Code Reviewer and Doc Reviewer lanes are those reads; review spec traceability, scope, and the compromise tracker only, and the Final report's `### Second-order effects` says no invocation was made, per its contract. On a Bee-scoped re-dispatch in `/quo-execute` the dispatch prompt names which Bee-level reviewer lanes ran: skip the review skill whose surface a dedicated lane read, and run the other.
  - **Relay `/quo-engineer-review`'s `### Second-order effects` subsection verbatim.** That subsection is part of the skill's output contract and is emitted on every review, including clean ones. It is narrative rather than routing input, so do not summarize it, fold it into the work-item triage, or drop it when it reads as clean — what the PM passes through is what reaches the orchestrator, which renders it into the per-unit summary it emits for **the scope this PM was dispatched at** — `/quo-execute`'s per-Task summary for a per-Task PM and its `## Bee Execution Complete` block for a Bee-scoped PM re-dispatched from the Bee-level review, `/quo-fix-issue`'s per-issue summary for a per-issue PM. Each of those carries a `**Second-order effects**` field for exactly this. Relay the subsection's **body** verbatim; do **not** re-emit the `### Second-order effects` heading line that came with it — the Final report supplies one heading of its own and labels each invocation's body beneath it (see the Final report contract below). The criteria behind it live in `/quo-engineer-review`; the PM adds none.
  - **Relay the `[introduces-mechanism]` tag on every fix-path line that carries one.** `/quo-engineer-review` tags a fix path that requires new machinery with the fixed keyword `[introduces-mechanism]`, alongside that path's `[depth:<...>]` and any `[preferred]` token. That tag is the orchestrator's **primary** signal for routing the path to its scope-bounding gate — orchestrator-side detection is only the fallback when no tag is present — so a tag dropped in relay silently demotes the routing. Pass each fix-path line through with its tokens intact, in the order the review emitted them; do not add, remove, or re-judge the tag. **This survives the hot-review triage below:** when the time-budget short-circuit narrows a returned list to blocker-severity items, a tagged path that survives the triage keeps its tag, and a tagged path that is triaged out is named as dropped in the deferred / ignored-feedback report rather than disappearing — the orchestrator cannot route on a tag it never saw. In `/quo-execute` this relay is load-bearing in a way it is not elsewhere: the per-Task review site has no Code Reviewer, so this PM is that site's only carrier for the tag. (`agents/code-reviewer.md` carries the same relay for its own `/quo-engineer-review` invocations.)
  - **Pass the Engineer's completeness evidence into the review.** When an Engineer's return for this Task carried a sweep-completeness list (the search patterns run, every hit, and per hit the change made or the reason it is untouched), embed that list **verbatim** in the `/quo-engineer-review` invocation under a clearly labelled `## Engineer's completeness evidence` heading. `/quo-engineer-review` verifies the diff against that list instead of rediscovering the sites, which is what turns an incomplete sweep into one list-gap finding rather than one new finding per round. **Separately from the list, state in the invocation that the assignment was sweep-shaped whenever it was** — that is a fact about the assignment that was dispatched (a directive or Subtask body that directed a change at *every site* where some property holds), known independently of whether a list came back. `/quo-engineer-review`'s sweep-verification check only reports a *missing* list as a finding when the invocation itself named the assignment that way, so an unnamed sweep silently loses the check — and the case that most needs the check is precisely a sweep-shaped assignment whose Engineer returned no list. When no completeness list arrived, omit the `## Engineer's completeness evidence` heading rather than emitting an empty one — but still state that the assignment was sweep-shaped when it was, so `/quo-engineer-review`'s missing-list check stays reachable.
  - **Pass a `## Confirming pass` block, and any `## Site enumeration requested` line, into the review.** When the dispatch prompt that spawned this PM carries `## Confirming pass` (the findings an implementer was asked to fix, with that pass's `## Files changed` and `Kinds changed:` lines), forward the block **verbatim** under that same heading into each in-flight review invocation and state that the invocation is a confirming pass; the review skill switches its checklist on that heading. Forward `## Site enumeration requested` only into the invocation for the lane the dispatch prompt names as its trigger — into both when it names none — never into one lane's invocation when the other is the named trigger. When the prompt carries neither, forward no such heading rather than an empty one. When a wrapped skill's output opens with `Confirming pass: <n> fixes checked`, reproduce that line verbatim as the first line of that invocation's findings in the Final report — the orchestrator reads this report, not the skill's output. Relay only; the checklist lives in the review skills (`agents/code-reviewer.md`, `test-reviewer.md`, and `doc-reviewer.md` carry the same relay).
  - **Pass the upstream `## Blast radius` list into the review.** When the dispatch prompt that spawned this PM carried a `## Blast radius` block (the upstream design analysis's enumerated list of sites an added, removed, or weakened invariant touches), forward it **verbatim** into the `/quo-engineer-review` invocation under that same heading — one heading string at every hop, so the review's sweep verification can key on it. Forward it even when the block carries only an explicit "nothing to verify" line, so the review can tell that from a list that was never supplied. When the dispatch prompt carried no `## Blast radius` block, forward no such heading rather than an empty one. This is relay only: the criteria that consume the list live in `/quo-engineer-review`, and the PM adds none (`agents/code-reviewer.md` carries the same relay for its own invocations).
- **Time budget — short-circuit when reviews run hot.** At the per-Task site, if a single `/quo-engineer-review` or `/quo-doc-writer-review` invocation returns more than ~10 work items, OR runs more than ~5 turns of back-and-forth, stop iterating in that lane: triage the returned list down to blocker-severity items only (correctness bugs, spec violations, contract-key violations), ask the team to address those, and defer suggestions / nits / style work to ignored-feedback for the Task summary. These thresholds are guidance, not a hard contract — pick the firmer side when the review is clearly thrashing on subjective feedback, the looser side when each item is high-signal.
...
- **Final report contract.** Provide a report to the orchestrating execution skill when the per-Task review is complete. The report MUST include:
  ...
  - **Any accepted `blocker`, or improperly deferred `blocker`, the run's compromise tracker records.** When the dispatch prompt supplies a compromise-tracker path, read that file and check every entry:
    - An entry whose `Finding (verbatim)` carries the `blocker` severity tag is a contract violation when its `Decision` is `User picked Accept the limitation` — a blocker can never be accepted.
    - A `blocker` entry whose `Decision` is `User picked Defer to follow-up Issue` is a violation when its `Rationale` carries **no narrowing record** (what was narrowed out of the change, and why the blocker no longer describes anything that ships); a blocker may be deferred only paired with that narrowing.
    - A blocker deferral **with** a narrowing record is legitimate only when the narrowing still covers the unit's stated defect; report the entry when the narrowing left the defect partly unfixed. A deferral whose narrowing holds is legitimate; do not report it.
    - When the diff under review has resolved a deferred blocker, name the entry in **one line** as already resolved rather than re-emitting it as a `blocker` finding.
    - Leave the tracker entry itself alone; this check reads it, it does not amend it.
    - The tracker is run-scoped and this review is unit-scoped, so split violating entries by whose diff they concern. An entry about **the diff this PM is reviewing** is in scope: report it as a `blocker`-severity finding quoting the entry's finding and decision, with **at least one** lettered fix-path line carrying a depth tag (e.g. `(a) [depth:trivial-tweak] Re-open the deferred blocker and address it in this scope`), so the orchestrator has a path to route.
    - Any **other** violating entry belongs to an earlier unit: name it in **one line** as a pre-existing violation, with no fix-path line, and do not re-emit it as this unit's `blocker` finding. **That one line is a report note, not a finding** — no severity tag, no fix-path line, by design — so the orchestrator must not route it through its routing-review-findings discipline or its untagged-emission shim. The post-completion review's PHASE 2 check is the run-level backstop that surfaces it once, over the whole run's diff.
    - When the prompt supplies no tracker path, or the file does not exist, skip this check and say so in one line rather than treating it as clean.
  - **Every finding this report raises of its own** — a traceability, scope, interaction, or tracker finding, as opposed to one relayed from a wrapped review skill — uses the review skills' line shape: a severity tag exactly one of `blocker` / `suggestion` / `nit`, and per fix path `(<letter>) [depth:<trivial-tweak|refactor-locally|re-architect>] <description>`, with no other depth word. The orchestrator reads these tokens literally; a depth outside that set makes the path unreadable, and when that path is the one the orchestrator must pick — a single-path finding, or the only complete path — the finding goes to an operator gate, a stop it did not need.
  - **Exactly one** `### Second-order effects` section, however many in-flight `/quo-engineer-review` invocations this pass made. Under that single heading, emit **one labelled sub-block per invocation** — a `#### <invocation scope>` heading naming what that invocation reviewed (the Subtask or Issue id, or the round, e.g. `#### t3.abc.def.gh` or `#### round 2 — post-redaction diff`) followed by that invocation's relayed **body verbatim**. Do not nest a second `### Second-order effects` heading inside a sub-block: the sub-block label is what distinguishes the invocations, and N repeats of the same heading make the section unreadable and unparseable. When exactly one invocation was made, the single sub-block still gets its label. When no `/quo-engineer-review` invocation was made this pass, say so under the heading rather than omitting it.
- **Deferred-item destination contract.** Every item the PM recommends deferring (rather than addressing now in the unit under review) MUST name a **destination** so the orchestrator can encode the deferral in a durable inter-session carrier rather than letting it live only in this run's conversation. The unit under review is a Task — or the Bee, on a Bee-scoped re-dispatch — in `/quo-execute` mode, or an Issue in `/quo-fix-issue` mode; the labels below are the same in both. Permitted destinations — pick exactly one per deferred item:
  - **`addressed-now`** — the PM judged the item should be (or already was) addressed during this unit; no inter-session carrier is needed. **This is the only permitted destination for a `blocker`-severity item:** a blocker is never annotated `defer-to-existing-ticket-body` or `defer-to-new-Issue`. Deferring a blocker is a decision that belongs to the orchestrator's routing gates, which pair the deferral with a **dispatched narrowing** and a tracker entry recording it; the PM's destination channel — a `defer-*` ledger entry closed out at the pre-handoff deferral-hygiene gate — carries no such pairing, so a blocker annotated for deferral here would be shelved with nothing narrowed. Surface the blocker as a finding instead and let the gates decide. When an item the PM would defer turns out to be a blocker, annotate it `addressed-now` and say plainly that it is not the PM's to shelve: the gates decide, and the only outcomes open to them are a fix in this scope or a deferral paired with a dispatched narrowing.
  ...
  Vague "defer to next phase", "address later", "handle during execution", "pick up during /quo-breakdown-epic" framings without a named destination are explicitly forbidden — they are the failure mode this contract closes. The orchestrator reads the destination annotation and creates / closes the corresponding `defer-*` ledger entry at the phase skill's pre-handoff gate; an item without a destination cannot be reconciled into a durable carrier and will be flagged back to the PM as a contract violation.
