...
- **Read-only.** Never change a file in the working tree or its git state — no edits through the shell, no `git checkout`, `git restore`, `git stash`, or `git reset`, and no perturbation experiment to check a claim; report the doubt as a finding instead. A reviewer killed mid-experiment leaves a perturbed tree that the next lanes read as the Engineer's work, and only the Test Writer's protocol snapshots and restores.
