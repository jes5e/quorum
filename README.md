...
| `/quo-fix-issue` | Fix one or more issue tickets. Argument shapes: interactive (no args), `all`, single bees ID, list of bees IDs, single `<url>`, or a mixed `<id>` + `<url>` list. URL tokens (`^https?://`) trigger file-then-fix routing through `/quo-file-issue` (same dedupe and `WebFetch` fallback apply) and the resolved ticket IDs substitute the URLs *in place* in the working list, preserving the user-supplied prerequisite ordering. Dispatches the same ephemeral background subagents as `quo-execute` but at issue scope, plus an Analyst design-analysis gate that runs per Issue before the Engineer dispatch. At end-of-run, emits copy-paste-ready `gh issue close ...` recommendations for any fixed Issues whose `reference_materials` carry a `github-issue` resolver — the skill never runs `gh issue close` itself. |
